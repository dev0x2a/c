...
static bool exec_global(
    const char **const ibufpp, const int gflags, const bool interactive);
                        ...
                        const bool isglobal)
{
  ...
  *ibufpp = skip_blanks(*ibufpp);
  ...
  case 'c':
    ...
    if (second_addr == 0)
      second_addr = 1;
    if (!check_current_addr(addr_cnt) || !get_command_suffix(ibufpp, &gflags))
      return ERR;
    if (!isglobal)
      clear_undo_stack();
  ...
  case 'h':
  case 'H':
    if (unexpected_address(addr_cnt) || !get_command_suffix(ibufpp, &gflags))
      return ERR;
    if (c == 'H')
      verbose = !verbose;
  ...
  case 'W':
    ...
    addr = write_file(fnp[0] ? fnp : def_filename, (c=='W') ? "a" : "w",
                      first_addr, second_addr);
    if (addr < 0)
      return ERR;
    if (addr == last_addr())
