/*
 * hlife0.96 by Radical Eye Software.
 *
 * Usage:
 ...
 *   order from the most significant bit.  So bit0x8000 is the upper
 *   left(or northwest)bit,and bit0x1000 is the upper right bit,and
 *   so on.
 }}} */
struct leaf{
   struct node *next;              /* hash link */
 ...
 *  0x20,the ne bit at0x10,the sw bit at0x2,and the se bit at0x1.
 *
...
struct leaf *find_leaf(unsigned short nw,unsigned short ne,
   ...
   for(p=(struct leaf*)hashtab[h]; p; p=(struct leaf*)p->next){
      if(nw==p->nw&&ne==p->ne&&sw==p->sw&&se==p->se&&!is_node(p)){
#ifdef TRACE
         hits++;
#endif
         if(pred){
...
struct node *getres(struct node *n,int depth){
   if(n->res==0){
      ...
      if(ngens>=depth){
         ...
         if(halvesdone<1000)halvesdone++;
         if(is_node(n->nw)){
            n->res=dorecurs_half(n->nw,n->ne,n->sw,n->se,depth);
         }else if(ngens==0){
            n->res=(struct node*)dorecurs_leaf_quarter(n->nw,n->ne,n->sw,n->se);
         }else{
            n->res=(struct node*)dorecurs_leaf_half(n->nw,n->ne,n->sw,n->se);
         }
      }
      pop(sp);
   }
 ...
 *   neighbor counts.
 */
int wolfram=-1; /* not used if -1; else0..255 */
 ...
 */
int base[200]; /* for first 100 numbers0..99 */
/*
 *   Give me a new small multiprecision number for0..99.
 */
...
void unpack8x8(unsigned short nw,unsigned short ne,unsigned short sw,
   ...
   *top=((nw&0xf000)<<16)|(((ne&0xf000)|(nw&0xf00))<<12)|
     (((ne&0xf00)|(nw&0xf0))<<8)|(((ne&0xf0)|(nw&0xf))<<4)|(ne&0xf);
   *bot=((sw&0xf000)<<16)|(((se&0xf000)|(sw&0xf00))<<12)|
     (((se&0xf00)|(sw&0xf0))<<8)|(((se&0xf0)|(sw&0xf))<<4)|(se&0xf);
}
/*
...
void writeout(char *filename,int argc,char *argv[]){
   ...
   fprintf(macro,"[M1](hlife0.96)");
   for(i=1; i<argc; i++)fprintf(macro," %s",argv[i]);
   fprintf(macro,"\n");
   cellcounter=0;
...
void do_gc(int why){
   ...
   for(i=0; i<gsp; i++)gc_mark(stack[i]);
   hashpop=0;
...
void clearcache(struct node *n,int depth,int clearto){
   if(!marked(n)){
      ...
      if(depth>3){
         ...
         if(n->res)clearcache(n->res,depth,clearto);
      }
      if(depth>=clearto)n->res=0;
   }
}
/*
...
void new_ngens(int newval){
   ...
   fprintf(stderr,"<");
   if(newval<clearto)clearto=newval;
   clearto++;/* clear this depth and above */
   if(clearto<3)clearto=3;
   ngens=newval;
   for(i=0; i<hashprime; i++)
   ...
   for(p=nodeblocks; p; p=p->next)
      for(pp=p+1,i=1; i<1001; i++,pp++)clearmark(pp);
   fprintf(stderr,">");
   halvesdone=0;
}
/*
...
int *parseint(int **what,char *s){
   ...
   i=strlen(s);
   while(gensize*9<=i+9)biggergennums();
   p=*what;
   v=0;
   if(i>1&&*s=='0')return(0);
   p[(i+8)/9]=-1;
   while(i>0){
      if('0'<=*s&&*s<='9')v=10*v+*s++-'0';
      else return(0);
      i--;
      if(i%9==0){
         p[i/9]=v;
...
int main(int argc,char *argv[]){
   ...
   int oargc=argc;
   char **oargv=argv;
   puts("This is hlife0.96 Copyright 2001-2005 Radical Eye Software");
   biggergennums();
   maxgens[0]=0;
   maxgens[1]=-1;
   incgens[0]=1;
 ...
 */
   readpattern();
   if(!nopop)printf("Start population is %s\n",population(root));
   if(opts==0||opts&(1<<('t'-'a'))){ /* do 1,2,4,8,16,32,64,128,... */
      ...
      while(1){
         ...
         if((opts&(1<<('m'-'a')))&&bignumcmp(gen,maxgens)>=0)break;
      }
#endif
   ...
   if(outfile)writeout(outfile,oargc,oargv);
#ifdef UI
   UI(root);
#endif
/* printf("Hash pop %d\n",hashpop); */
