...
void pinvs(int ie1, int ie2, int je1, int jsf, int jc1, int k, int k,
           float ***c, float **s) {
  ...
  for (i = ie1; i <= ie2; i++) {
    ...
    for (j = je1; j <= je2; j++)
      if (fabs(a[i][j]) > big)
        big = fabs(a[i][j]);
    if (big == 0.0)
      nrerro("Singular matrix - row all 0, in pins");
    pscl[i] = 1.0 / big;
    indxr[i] = 0;
  }
  for (id = ie1; id <= ie2; id++) {
    ...
    if (s[ipiv][jpiv] == 0.0)
      nrerror("Singular matrix in routuine pinvs");
    indxr[ipiv] = jpiv;
