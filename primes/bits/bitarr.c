/* Bit Vectors in C
  ...
  IMPORTANT NOTE: A version of this code appeared in Dr. Dobb's Journal
  issue #233 (August 1995) Volume 20 Issue 8 in an article entitled
  `Implementing Bit Vectors in C' by James Blustein Pages 42, 44, 46 (article)
  and pages 96, 98-100 (code)
...
{ /* PRE: `arr' is a bit array with at least `size' elements.  Either
  ...
  if ((NULL!=dest) || (NULL!=(dest=(char *)malloc(size+1)))) {
    for (i=0; i<size; ++i) {
...
{ /* PRE:  `first'  is a bit array of at least `size_first'  elements
  ...
  NOTE: This runs faster if the `first' array
        is not smaller than `second' */
  register elem_t i;
  elem_t numints;
  ...
  if ((NULL==*result) && (NULL==(*result=ba_new(bv[largest].size)))) {
    return (FALSE); /* can't get memory, so can't continue */
  } else {
    numints = NELEM(size_second, (BITS_SZ));
    for (i=0; i<numints; ++i) {
      (*result)[i] = (bv[smallest].vector[i] & bv[largest].vector[i]);
    }
...
bool ba_union(bit first[], bit second[], bit *result[], const elem_t size_first,
...
bool ba_diff(bit first[], bit second[], bit *diff[], const elem_t size_first,
             const elem_t size_second)
{ /* PRE:  `first'  is a bit array of at least `size_first'  elements
          `second' is a bit array of at least `size_second' elements
  ...
  bv[smallest].vector = second;
  first_is_biggest(bv, &largest, &smallest);
  if ((NULL==*diff) && (NULL==(*diff=ba_new(bv[largest].size)))) {
    return (FALSE);
  } else {
...
static void first_is_biggest(BitVector bv[2], unsigned *big, unsigned *small)
{
