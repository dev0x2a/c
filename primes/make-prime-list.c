...
{ /* Number of bits per integer literal.  8 is too many, because uintmax_t is
     32 bits on some machines so we cannot shift by 32 bits; so use 7 */
...
int main(int argc, char *argv[])
{
  ...
  if (!(limit&1)) /* Make limit odd */
    limit--;
  size = (limit-1)/2;
  sieve = xalloc(size); /* sieve[i] represents 3+2*i */
  memset(sieve, 1, size);
