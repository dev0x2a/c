...
static void get_sieve_params(
    uint32 bits, 
		sieve_param_t *params);

static void build_factor_base(
    mp_t *n,
...
static uint8 choose_multiplier(
    mp_t *n,
		prime_list_t *prime_list,
		uint32 fb_size);
	...
	*out_multiplier = choose_multiplier(n, prime_list, fb_size);
	...
	for (i=1,j=MIN_FB_OFFSET+1; i<num_primes; ++i) {
		...
		if (nmodp==0 || mp_legendre_1(nmodp, prime)==1) {
			fbptr->prime = prime;
			fbptr->logprime = (uint8)(log((double)prime)/M_LN2+.5);
...
static void get_sieve_params(uint32 bits, sieve_param_t *params)
{
    ...
    (((double)low->fb_size*j+(double)high->fb_size*i)/dist+0.5);
	params->large_mult = (uint32)
    (((double)low->large_mult*j+(double)high->large_mult*i)/dist+0.5);
	params->sieve_size = (uint32)
    (((double)low->sieve_size*j+(double)high->sieve_size*i)/dist+0.5);
}
