...
/* An implementation of the Self-Initializing Multiple 
   Polynomial Quadratic Sieve algorithm for integer actorization */
#include <common.h>

/*------------------- SIEVE RELATED DECLARATIONS ---------------------*/
/* There's a limit to how many factors can
   contribute to SIQS polynomials */
...
typedef struct {
	...
	uint32 root1,	     /* the two square roots of an MPQS */
         root2,	     /* polynomial (INVALID_ROOT means don't use) */
         recip;      /* integer reciprocal of prime */
} fb_t;
...
typedef struct relation_t {
	uint32 sieve_offset,	 /* value of x in a*x+b */
         poly_idx,	     /* pointer to this relation's MPQS poly */
         num_factors;	   /* size of list of factors */
...
typedef struct {
	uint32 num_alloc,     /* total list size this hashtable position */
         num_used;      /* number occupied at this hashtable position */
...
typedef struct {
	uint32 bits,       /* size of integer this config info applies to */
         fb_size,    /* number of factor base primes */
         large_mult, /* the large prime multiplier */
         sieve_size; /* the size of the sieve (actual sieve is 2x this) */
} sieve_param_t;
...
typedef struct {
	...
	uint32 num_sieve_blocks,  /* number of sieve blocks in sieving interval*/
         poly_block,        /* number of FB primes in a block of work */
         fb_block;          /* number of polynomials simultaneously
                               sieved */

	uint32 sieve_small_fb_start, /* starting FB offset for sieving */
         sieve_large_fb_start;

	uint32 tf_small_recip1_cutoff,
         tf_small_recip2_cutoff,
         tf_med_recip1_cutoff,
         tf_med_recip2_cutoff,
         tf_large_cutoff;
	...
	uint32 cycle_table_size,   /* number of vertices filled in the table */
         cycle_table_alloc,  /* number of cycle_t structures allocated */
         *cycle_hashtable,   /* hashtable to index into cycle_table */
         components,         /* connected components (see relation.c) */
         vertices;           /* vertices in graph (see relation.c) */
} sieve_conf_t;

/* attempt to trial factor one sieve value */
uint32 check_sieve_val(
    sieve_conf_t *conf,
    int32 sieve_offset, 
		uint32 bits,
		mp_t *a,
...
typedef uint32 (*qs_core_sieve_fcn)(
    sieve_conf_t *conf,
		uint32 poly_start,
		uint32 num_poly);

#define DECLARE_SIEVE_FCN(name) \
	uint32 name(sieve_conf_t *conf, uint32 poly_start, uint32 num_poly);
...
/* pull out the large primes from a relation read from the savefile */
void read_large_primes(char *buf, uint32 *prime1, uint32 *prime2);

/* given the primes from a sieve relation, add
   that relation to the graph used for tracking cycles */
void add_to_cycles(
    sieve_conf_t *conf, 
		uint32 large_prime1, 
		uint32 large_prime2);
...
/* Initialize/free polynomial scratch data. Note that poly_free
   can only be called after the relation filtering phase */
