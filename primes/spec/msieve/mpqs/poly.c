...
void poly_init(sieve_conf_t *conf, uint32 sieve_size)
{
	...
	conf->next_poly_action = 
    (uint8 *)xmalloc(num_derived_poly*sizeof(uint8));
	conf->curr_b = 
    (signed_mp_t *)xmalloc(num_derived_poly*sizeof(signed_mp_t));
	conf->poly_b_small[0] = 
    (uint32 *)xmalloc(conf->sieve_large_fb_start*num_factors*sizeof(uint32));
	...
	if (conf->fb_size > conf->sieve_large_fb_start) {
		conf->poly_b_array =
      (uint32 *)xmalloc(num_factors*sizeof(uint32)*
    ...
    if (prime<100 && conf->multiplier%prime==0) {
        ...
        i==poly_factors[curr_poly_factor]) {
			...
			if (i >= conf->sieve_large_fb_start) {
				...
				for (j=0; j<num_factors; ++j) {
					b_modp = mp_mod_1(&poly_tmp_b[j].num, prime);
					poly_b_small[j][i] =
            mp_modmul_1(a_modp, b_modp, prime);
				}
			}
		}
