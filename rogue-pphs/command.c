...
{ register char ch;
  ...
  while(ntimes--){
	...
	if(!((running||count)&&jump))draw(cw);/* Draw screen */
	take=0;
	after=TRUE;
	/*
	 * Read command or continue run
	 */
	if(wizard)waswizard=TRUE;
	if(!no_command){
    if(running)ch=runch;
    else if(count)ch=countch;
    else{
      ch=readchar(cw);
      if(mpos!=0&&!running)msg("");/* Erase message if its there */
    }
  }else ch=' ';
  if(no_command){
    if(--no_command==0)msg("You can move again.");
  }else{
	  /*
	   * check for prefixes
	   */
    ...
    when 'f':
		if(!get_dir())after=FALSE;
		else flirt(delta.y,delta.x);
    when 'g':
		if(!get_dir())after=FALSE;
    else gift(delta.y,delta.x);
    when 'E':embrace();
    ...
    when ' ':;/* Rest command */
		...
		if(wizard){
			...
			if(wizard=passwd()){
			  msg("You are suddenly as smart as Ken Arnold in cavern #%d",dnum);
 			  wizard=TRUE;
			  waswizard=TRUE;
			}else msg("Sorry");
    }
  ...
  if(after){
	  ...
	  do_daemons(AFTER);
	  do_fuses(AFTER);
	if(ISRING(LEFT,R_SEARCH))search();
	else if(ISRING(LEFT,R_TELEPORT)&&rnd(100)<2)teleport();
	if(ISRING(RIGHT,R_SEARCH))search();
	else if(ISRING(RIGHT,R_TELEPORT)&&rnd(100)<2)teleport();
  }
}
/*
 * quit:
...
{ /*
   ...
   */
  if(signal(SIGINT,quit)!=&quit)mpos=0;
  msg("Really quit?");
  draw(cw);
  if(readchar(cw)=='y'){
	  clear();
...
{ register struct h_list *strp=helpstr;
  ...
  if(helpch!='*'){
    ...
    if(strp->h_ch!=helpch)msg("Unknown character '%s'",unctrl(helpch));
    return;
  }
  /*
...
{ register char ch,*str;
  ...
  if(ch==ESCAPE){
	  msg("");
    return;
  }if(isalpha(ch)&&isupper(ch))str=monsters[ch-'A'].m_name;
  else switch(ch){
	case '|':case '-':
    str="wall of a room";
	when GOLD:str="gold";
 ...
 */
d_level()
{ if(winat(hero.y,hero.x)!=STAIRS)msg("I see no way down.");
  else{
    level++;
    new_level();
}}
...
otherwise:
  ...
  if(terse)msg("Call it:");
  else msg("What do you want to call it? ");
  strcpy(prbuf,elsewise);
  if(get_str(prbuf,cw)==NORM){
    if(guess[obj->o_which]!=NULL)free(guess[obj->o_which]);
	guess[obj->o_which]=malloc((unsigned int)strlen(prbuf)+1);
	if(guess[obj->o_which]!=NULL)strcpy(guess[obj->o_which],prbuf);
  }
}
