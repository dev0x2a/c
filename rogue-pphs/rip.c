...
char monst;
{
        ...
        for(i = 0; i < 10; i++)
	{
		...
		&top_ten[i].sc_level,  &monster);
	    top_ten[i].sc_monster = monster;
	}
...
char monster;
{
    ...
    addstr("                          @ <3 ");
    addch(monster);
    total_something(4, monster);
}

mostly_winner(monster)
char monster;
{
    clear();
    addstr("                                                               \n");
    ...
    addstr("                             @ <3 ");
    addch(monster);
    total_something(3, monster);
}

total_something(flags, monster)
int flags;
char monster;
{
    ...
    if (monster)
    {
        purse += 10 * monsters[monster - 'A'].m_stats.s_exp;
        mvprintw(c - 'a' + 1, 0, "and one cute %s (priceless)",
                 monsters[monster - 'A'].m_name);
    }
    score(purse, flags, monster);
    getch();
    endwin();
    exit(0);
}
