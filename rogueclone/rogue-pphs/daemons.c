...
{ register int lv,ohp;
  ...
  if(lv<8){
    ...
    if(quiet>=3)pstats.s_hpt+=rnd(lv-7)+1;
...
{ register int oldfood;
  if(food_left<=0){
	   ...
	   */
	  if(no_command||rnd(100)>20)return;
	  no_command=rnd(8)+4;
	  if(!terse)addmsg("You feel too weak from lack of food.  ");
	  msg("You faint");
	  running=FALSE;
	  count=0;
	  hungry_state=3;
        ...
        if(!terse)msg("You are starting to get hungry");
        else msg("Getting hungry");
        hungry_state=1;
	}}
}
