...
{ register struct thing *tp;
  ...
  if(did_hit){
    ...
    else mname=monsters[mn-'A'].m_name;
    if(roll_em(&pstats,&tp->t_stats,weap,thrown)){
	    did_hit=TRUE;
	    if(thrown)thunk(weap,mname);
	    else hit(NULL,mname);
	    if(on(player,CANHUH)){
	    ...
	    if(tp->t_stats.s_hpt>=0)killed(item,TRUE);
    }else
      if(thrown)bounce(weap,mname);
...
{ register char *mname;
  ...
  if(rnd(5)==0)mp->t_stats.s_int--;
  if(mp->t_stats.s_int>READY){
    const char *msgs[]={
      "The %s smiles at you.",
    ...
    if(mp->t_stats.s_int>rnd(INTERESTED)){
      const char *msgs[]={
        "The %s stares at you.",
        "The %s hesitates.",
        "The %s takes a step towards you,then back.",
      };
      msg(rndchoice(msgs),monsters[mp->t_type-'A'].m_name);
      return;
    }
    ...
    running=FALSE;
    quiet=0;
    if(mp->t_type=='M'&&off(player,ISBLIND))mp->t_disguise='M';
    if(on(player,ISBLIND))mname="it";
    else mname=monsters[mp->t_type-'A'].m_name;
    if(roll_em(&mp->t_stats,&pstats,NULL,FALSE)){
      if(mp->t_type!='E')hit(mname,NULL);
      if(pstats.s_hpt>=0)death(mp->t_type);/* Bye bye life ... */
      if(off(*mp,ISCANC))switch(mp->t_type){
        case 'R':
		    /*
		     * If a rust monster hits,you lose armor
		     */
		    if(cur_armor!=NULL&&cur_armor->o_ac>9){
          if(!terse)msg("Your armor appears to be weaker now. Oh my!");
          else msg("Your armor weakens");
          cur_armor->o_ac++;
        }
        when 'E':
		      /*
        ...
        when 'A':
          ...
          if(!save(VS_POISON))if(!ISWEARING(R_SUSTSTR)){
            chg_str(-1);
            if(!terse)msg("You feel a sting in your arm and now feel weaker");
            else msg("A sting has weakened you");
          }else
            if(!terse)msg("A sting momentarily weakens you");
     ...
     * it is.
     */
    if((mp!=NULL)&&(on(*mp,ISREGEN)&&rnd(100)>33))mp->t_stats.s_hpt++;
    if(fight_flush){
...
{ register char *cp;
  ...
  if(weap==NULL)cp=att->s_dmg;
  else if(hurl)if((weap->o_flags&ISMISL)&&cur_weapon!=NULL&&
      cur_weapon->o_which==weap->o_launch){
    cp=weap->o_hurldmg;
	  prop_hplus=cur_weapon->o_hplus;
	  prop_dplus=cur_weapon->o_dplus;
	}else cp=(weap->o_flags&ISMISL?weap->o_damage:weap->o_hurldmg);
  else{
    cp=weap->o_damage;
	  /*
    ...
    if(weap==cur_weapon){
	    ...
	    else if(ISRING(RIGHT,R_ADDHIT))hplus+=cur_ring[RIGHT]->o_ac;
    }
    ndice=atoi(cp);
    ...
    if(def==&pstats){
      ...
      else def_arm=def->s_arm;
	    if(ISRING(LEFT,R_PROTECT))def_arm-=cur_ring[LEFT]->o_ac;
	    else if(ISRING(RIGHT,R_PROTECT))def_arm-=cur_ring[RIGHT]->o_ac;
    }else def_arm=def->s_arm;
...
{ if(weap->o_type==WEAPON)
  ...
  else msg("You missed the %s.",mname);
}
/*
...
{ switch(obj->o_type){
	  case ARMOR:return obj->o_ac!=a_class[obj->o_which];
               when WEAPON:return obj->o_hplus!=0||obj->o_dplus!=0;
               when POTION:
    case SCROLL:
	  case STICK:
	  case RING:
  ...
  switch(tp->t_type){
    case 'F':
      ...
      when 'L':{
        ...
        if((rp=roomin(&tp->t_pos))==NULL)break;
        if(rp->r_goldval!=0||fallpos(&tp->t_pos,&rp->r_gold,FALSE)){
          rp->r_goldval+=GOLDCALC;
          if(save(VS_MAGIC))rp->r_goldval+=GOLDCALC+GOLDCALC+GOLDCALC+GOLDCALC;
