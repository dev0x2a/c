...
static unsigned char DIGEST_INFO_A[] = {0x30, 0x20, 0x30, 0x0c, 0x06,
...
static unsigned char *PADDING[] = {
    ...
    (unsigned char *)"\01",
    (unsigned char *)"\02\02",
    (unsigned char *)"\03\03\03",
    (unsigned char *)"\04\04\04\04",
...
static void R_EncodeDigestInfo PROTO_LIST((unsigned char *, int,
                                           unsigned char *));
static int R_CheckDigestInfo PROTO_LIST((unsigned char *, unsigned char *));

/* encrypt prototypes */

static int CipherInit PROTO_LIST((R_ENVELOPE_CTX *, int, unsigned char *, int,
                                  unsigned char *, int));
static void EncryptBlk PROTO_LIST((R_ENVELOPE_CTX *, unsigned char *,
...
void R_RSAEuroInfo(info) RSAEUROINFO *info; /* info structure */
{
...
int R_DigestInit(context, digesttype) R_DIGEST_CTX *context; /* new context */
...
int R_DigestUpdate(context, partIn,
                   partInLen) R_DIGEST_CTX *context; /* context */
...
int R_DigestFinal(context, digest,
                  digestLen) R_DIGEST_CTX *context; /* context */
...
int R_SignInit(context, digesttype) R_SIGNATURE_CTX *context; /* new context */
int digesttype; /* message-digest algorithm */
{
  return ((digesttype == DA_SHS)
              ? RE_DIGEST_ALGORITHM
              : R_DigestInit(&context->digestContext, digesttype));
}

int R_SignUpdate(context, partIn,
                 partInLen) R_SIGNATURE_CTX *context; /* context */
unsigned char *partIn;                                /* next data part */
unsigned int partInLen; /* length of next data part */
{
  return ((context->digestContext.digestAlgorithm == DA_SHS)
              ? RE_DIGEST_ALGORITHM
              : R_DigestUpdate(&context->digestContext, partIn, partInLen));
}

int R_SignFinal(context, signature, signatureLen,
                privateKey) R_SIGNATURE_CTX *context; /* context */
...
R_RSA_PRIVATE_KEY *privateKey; /* signer's RSA private key */
{
  ...
  if (context->digestContext.digestAlgorithm == DA_SHS)
    ...
    if ((status = R_DigestFinal(&context->digestContext, digest, &digestLen)) ==
        0) {
      R_EncodeDigestInfo(digestInfo, context->digestContext.digestAlgorithm,
                         digest);

      if (RSAPrivateEncrypt(signature, signatureLen, digestInfo,
                            DIGEST_INFO_LEN, privateKey) != 0) {
        status = RE_PRIVATE_KEY;
      } else {
        /* Reset for another verification. */
        R_DigestInit(&context->digestContext,
                     context->digestContext.digestAlgorithm);
      }
    }
...
int R_VerifyInit(context,
                 digestAlgorithm) R_SIGNATURE_CTX *context; /* new context */
int digestAlgorithm; /* message-digest algorithm */
{
  return ((digestAlgorithm == DA_SHS)
              ? RE_DIGEST_ALGORITHM
              : R_DigestInit(&context->digestContext, digestAlgorithm));
}

int R_VerifyUpdate(context, partIn,
                   partInLen) R_SIGNATURE_CTX *context; /* context */
unsigned char *partIn;                                  /* next data part */
unsigned int partInLen; /* length of next data part */
{
...
int R_VerifyFinal(context, signature, signatureLen,
                  publicKey) R_SIGNATURE_CTX *context; /* context */
unsigned char *signature;                              /* signature */
unsigned int signatureLen;                             /* length of signature */
R_RSA_PUBLIC_KEY *publicKey; /* signer's RSA public key */
{
  ...
  if (!status) {
    if ((status = R_DigestFinal(&context->digestContext, digest, &digestLen)) ==
      ...
      if (RSAPublicDecrypt(originalDigestInfo, &originalDigestInfoLen,
                           signature, signatureLen, publicKey) != 0) {
        status = RE_PUBLIC_KEY;
      } else { /* Check the digest out */
        if ((originalDigestInfoLen != DIGEST_INFO_LEN) ||
            R_CheckDigestInfo(originalDigestInfo, digestInfo) ||
            (R_memcmp((POINTER)originalDigestInfo, (POINTER)digestInfo,
                      DIGEST_INFO_LEN)))
          status = RE_SIGNATURE;
        else
          /* Reset for another verification. */
...
int R_SealInit(context, encryptedKeys, encryptedKeyLens, iv, publicKeyCount,
               publicKeys, encryptionAlgorithm,
               randomStruct) R_ENVELOPE_CTX *context; /* new context */
...
R_RANDOM_STRUCT *randomStruct;  /* random structure */
{
  ...
  if ((status = R_GenerateBytes(key, keyLen, randomStruct)) == 0) {
    if ((status = R_GenerateBytes(iv, 8, randomStruct)) == 0) {
      ...
      if ((status = CipherInit(context, encryptionAlgorithm, key, keyLen, iv,
                               1)) == 0) {
        for (i = 0; i < publicKeyCount; ++i) {
          if (RSAPublicEncrypt(encryptedKeys[i], &encryptedKeyLens[i], key,
                               keyLen, publicKeys[i], randomStruct)) {
            status = RE_PUBLIC_KEY;
            break;
          }
        }
...
int R_SealUpdate(context, partOut, partOutLen, partIn,
                 partInLen) R_ENVELOPE_CTX *context; /* context */
...
unsigned int partInLen;   /* length of next data part */
{
  ...
  if (partInLen < temp) {
    ...
    *partOutLen = 0;
    R_memcpy((POINTER)(context->buffer + context->bufferLen), (POINTER)partIn,
             partInLen);
...
int R_SealFinal(context, partOut,
                partOutLen) R_ENVELOPE_CTX *context; /* context */
...
unsigned int *partOutLen; /* length of last encrypted data part */
{
  ...
  R_memset((POINTER)(context->buffer + context->bufferLen), (int)padLen,
           padLen);
  EncryptBlk(context, partOut, context->buffer, 8);
  *partOutLen = 8;
...
int R_OpenInit(context, encryptionAlgorithm, encryptedKey, encryptedKeyLen, iv,
               privateKey) R_ENVELOPE_CTX *context; /* new context */
int encryptionAlgorithm;       /* data encryption algorithm */
...
int R_OpenUpdate(context, partOut, partOutLen, partIn,
                 partInLen) R_ENVELOPE_CTX *context; /* context */
...
unsigned int partInLen;   /* length of next encrypted data part */
{
  ...
  if (partInLen <= tempLen) {
    /* Just accumulate into buffer. */
    *partOutLen = 0;
    R_memcpy((POINTER)(context->buffer + context->bufferLen), partIn,
             partInLen);
...
int R_OpenFinal(context, partOut,
                partOutLen) R_ENVELOPE_CTX *context; /* context */
    ...
    *partOutLen = 0;
    ...
    if (context->bufferLen != 8) {
      ...
      if (padLen == 0 || padLen > 8)
        ...
        if (R_memcmp((POINTER)&lastPart[8 - padLen], PADDING[padLen], padLen) !=
            0)
          status = RE_KEY;
        else
...
int R_SignPEMBlock(
    ...
    content, contentLen, recode, digestAlgorithm,
    privateKey) unsigned char *encodedContent; /* encoded content */
...
R_RSA_PRIVATE_KEY *privateKey;                 /* signer's RSA private key */
{
  ...
  R_EncodePEMBlock(encodedSignature, encodedSignatureLen, signature,
                   signatureLen);

  return (ID_OK);
}

int R_SignBlock(signature, signatureLen, block, blockLen, digestAlgorithm,
                privateKey) unsigned char *signature; /* signature */
unsigned int *signatureLen;                           /* length of signature */
...
int R_VerifyPEMSignature(content, contentLen, encodedContent, encodedContentLen,
                         ...
                         digestAlgorithm,
                         publicKey) unsigned char *content; /* content */
unsigned int *contentLen;         /* length of content */
...
R_RSA_PUBLIC_KEY *publicKey;      /* signer's RSA public key */
{
  ...
  if (recode) {
    if (R_DecodePEMBlock(content, contentLen, encodedContent,
                         encodedContentLen))
  ...
  if (R_DecodePEMBlock(signature, &signatureLen, encodedSignature,
                       encodedSignatureLen))
    return (RE_SIGNATURE_ENCODING);

  return (R_VerifyBlockSignature(content, *contentLen, signature, signatureLen,
                                 digestAlgorithm, publicKey));
}

int R_VerifyBlockSignature(block, blockLen, signature, signatureLen,
                           digestAlgorithm,
                           publicKey) unsigned char *block; /* block */
unsigned int blockLen;       /* length of block */
unsigned char *signature;    /* signature */
unsigned int signatureLen;   /* length of signature */
int digestAlgorithm;         /* message-digest algorithm */
...
int R_SealPEMBlock(encryptedContent, encryptedContentLen, encryptedKey,
                   ...
                   iv, content, contentLen, digestAlgorithm, publicKey,
                   privateKey, randomStruct) unsigned char
    *encryptedContent;               /* encoded, encrypted content */
...
R_RANDOM_STRUCT *randomStruct;       /* random structure */
{
  ...
  if ((status = R_SignBlock(signature, &signatureLen, content, contentLen,
    ...
    if ((status = R_SealInit(&context, encryptedKeys, &encryptedKeyBlockLen, iv,
                             1, publicKeys, EA_DES_CBC, randomStruct)) == 0) {
...
int R_OpenPEMBlock(content, contentLen, encryptedContent, encryptedContentLen,
                   encryptedKey, encryptedKeyLen, encryptedSignature,
                   encryptedSignatureLen, iv, digestAlgorithm, privateKey,
                   publicKey) unsigned char *content; /* content */
unsigned int *contentLen;                             /* length of content */
...
R_RSA_PUBLIC_KEY *publicKey;        /* signer's RSA public key */
{
  ...
  if (R_DecodePEMBlock(encryptedKeyBlock, &encryptedKeyBlockLen, encryptedKey,
                       encryptedKeyLen) != 0) {
    status = RE_KEY_ENCODING;
  } else {
    if ((status = R_OpenInit(&context, EA_DES_CBC, encryptedKeyBlock,
                             encryptedKeyBlockLen, iv, privateKey)) == 0) {

      if ((status = R_DecryptOpenPEMBlock(&context, content, contentLen,
                                          encryptedContent,
        ...
        status =
            R_DecryptOpenPEMBlock(&context, signature, &signatureLen,
        ...
        if (status) {
          ...
          status =
              R_VerifyBlockSignature(content, *contentLen, signature,
                                     signatureLen, digestAlgorithm, publicKey);
      }
    }
  }
  /* Clear sensitive information. */
...
int R_DigestBlock(digest, digestLen, block, blockLen,
                  digestAlgorithm) unsigned char *digest; /* message digest */
unsigned int *digestLen; /* length of message digest */
unsigned char *block;    /* block */
unsigned int blockLen;   /* length of block */
int digestAlgorithm;     /* message-digest algorithm */
...
int R_DecryptOpenPEMBlock(context, output, outputLen, input,
                          inputLen) R_ENVELOPE_CTX *context; /* context */
...
unsigned int inputLen;   /* length */
{
  ...
  *outputLen = 0;
  ...
  for (i = 0; i < inputLen / 32; i++) {
    /* len is always 24 */
    if ((status = R_DecodePEMBlock(encryptedPart, &len, &input[32 * i], 32)) !=
        0)
      break;
  ...
  if (!status) /* Decode the last block. */
    if ((status = R_DecodePEMBlock(encryptedPart, &len, &input[32 * i],
                                   inputLen - 32 * i)) == 0) {
...
int R_EncryptOpenPEMBlock(context, output, outputLen, input,
                          inputLen) R_ENVELOPE_CTX *context; /* context */
...
unsigned int inputLen;   /* length */
{
  ...
  R_SealUpdate(context, encryptedPart, &lastPartLen, &input[24 * i],
               inputLen - 24 * i);
...
static void R_EncodeDigestInfo(digestInfo, digestAlgorithm,
                               digest) unsigned char *digestInfo;
int digestAlgorithm;
unsigned char *digest;
{
  if (!(digestAlgorithm == DA_SHS)) {
    ...
    R_memcpy((POINTER)&digestInfo[DIGEST_INFO_A_LEN + 1],
             (POINTER)DIGEST_INFO_B, DIGEST_INFO_B_LEN);
    R_memcpy((POINTER)digestInfo, (POINTER)DIGEST_INFO_A, DIGEST_INFO_A_LEN);
    R_memcpy((POINTER)&digestInfo[DIGEST_INFO_A_LEN + 1 + DIGEST_INFO_B_LEN],
             (POINTER)digest, 16);
  }
}
...
static int R_CheckDigestInfo(originaldigestInfo,
                             digestInfo) unsigned char *originaldigestInfo;
unsigned char *digestInfo;
{
  return (
      (originaldigestInfo[DIGEST_INFO_A_LEN] == digestInfo[DIGEST_INFO_A_LEN])
          ? ID_OK
          : RE_SIGNATURE);
}
...
static int CipherInit(context, encryptionAlgorithm, key, keyLen, iv,
                      encrypt) R_ENVELOPE_CTX *context;
 ...
 */
static void EncryptBlk(context, output, input, len) R_ENVELOPE_CTX *context;
...
static void RestartCipher(context) R_ENVELOPE_CTX *context;
{
  switch (context->encryptionAlgorithm) {
  case EA_DES_CBC:
