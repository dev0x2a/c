...
#define APPEND(a)                                                              \
  s = strtok(strcpy(t, a), " ");                                               \
  while (s)                                                                    \
    strcpy(seq[nbr++], s), s = strtok(0, " ");
#define INITSET(a, b)                                                          \
  memset(&seq, nbr = 0, sizeof(seq));                                          \
  APPEND(a)                                                                    \
  replicate("UDRLBF", 1);                                                      \
  replicate("UDFBRL", 1);                                                      \
  APPEND(b) SORT
#define MULTIPLY(a, b)                                                         \
  INITSET(a, b) replicate("FBLRDU", 1);                                        \
  replicate("RLUDFB", 2);                                                      \
  replicate("DUFBLR", 1);                                                      \
  SORT
...
#define CHECK(a)                                                               \
  (cube->color[i + a] -                                                        \
   "UFURUBULDFDRDBDLFRFLBRBLUFRURBUBLULFDRFDFLDLBDBR"[i + a])
...
HEURISTIC(hAll, EDGE(0, 24)
                    CORNER TEST(sum - 16 ? sum - 13 ? sum - 8 ? 3 : 1 : 2 : 2))
...
int main(int argc, char *argv[]) {
  ...
  rotColor[i] = "^om`FUNW]nl_VMXEpka\\hkneIOMKofil\
      ...
      hd_m[fp]RLVD\\ZeoCQKUYgn^ad[^HBDFcZ]`_bY\\EGAC"[i] -
                65;
