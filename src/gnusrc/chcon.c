...
#define AUTHORS proper_name("Russell Coker"), proper_name("Jim Meyering")
...
static int compute_context_from_mask(char const *context, context_t *ret) {
  ...
  if (!new_context) {
    error(0, errno, _("failed to create security context: %s"), quote(context));
    return 1;
  }

#define SET_COMPONENT(C, comp)                                                 \
  do {                                                                         \
    if (specified_##comp && context_##comp##_set((C), specified_##comp)) {     \
      error(0, errno, _("failed to set %s security context component to %s"),  \
...
int main(int argc, char **argv) {
  ...
  if (recurse) {
    if (bit_flags == FTS_PHYSICAL) {
      if (dereference == 1)
        error(EXIT_FAILURE, 0, _("-R --dereference requires either -H or -L"));
