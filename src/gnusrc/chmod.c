...
enum { NO_PRESERVE_ROOT = CHAR_MAX + 1, PRESERVE_ROOT, REFERENCE_FILE_OPTION };
...
static bool mode_changed(int dir_fd, char const *file,
                         char const *file_full_name, mode_t old_mode,
                         mode_t new_mode) {
...
static void describe_change(const char *file, mode_t old_mode, mode_t mode,
  ...
  case CH_NO_CHANGE_REQUESTED:
    ...
    printf(fmt, quoteaf(file), (unsigned long int)(mode & CHMOD_MODE_BITS),
           &perms[1]);
    return;
  default:
    abort();
  }
  printf(fmt, quoteaf(file), (unsigned long int)(old_mode & CHMOD_MODE_BITS),
         &old_perms[1], (unsigned long int)(mode & CHMOD_MODE_BITS), &perms[1]);
}
...
static bool process_file(FTS *fts, FTSENT *ent) {
  ...
  if (verbosity != V_off) {
    bool changed =
        (chmod_succeeded && mode_changed(fts->fts_cwd_fd, file, file_full_name,
                                         old_mode, new_mode));
    ...
    if (changed || verbosity == V_high) {
      enum Change_status ch_status =
          (!ok ? CH_FAILED
               : !chmod_succeeded
                     ? CH_NOT_APPLIED
                     : !changed ? CH_NO_CHANGE_REQUESTED : CH_SUCCEEDED);
  ...
  if (chmod_succeeded && diagnose_surprises) {
    ...
    if (new_mode & ~naively_expected_mode) {
      ...
      error(0, 0, _("%s: new permissions are %s, not %s"),
            quotef(file_full_name), new_perms + 1, naively_expected_perms + 1);
      ok = false;
    }
  }
