...
#define FTSENT_IS_DIRECTORY(E)                                                 \
  ((E)->fts_info == FTS_D || (E)->fts_info == FTS_DC ||                        \
   (E)->fts_info == FTS_DP || (E)->fts_info == FTS_DNR)
...
extern void chopt_free(struct Chown_option *chopt _GL_UNUSED) {
...
static void describe_change(const char *file, enum Change_status changed,
...
static enum RCH_status restricted_chown(int cwd_fd, char const *file,
  ...
  if (!(0 <= fd || (errno == EACCES && S_ISREG(orig_st->st_mode) &&
...
static bool change_file_owner(FTS *fts, FTSENT *ent, uid_t uid, gid_t gid,
  ...
  if (do_chown) {
    ...
    if (do_chown && !ok && !chopt->force_silent)
      error(0, errno, (uid != (uid_t)-1 ? _("changing ownership of %s")
                                        : _("changing group of %s")),
            quoteaf(file_full_name));
  }

  if (chopt->verbosity != V_off) {
    bool changed = ((do_chown && ok && symlink_changed) &&
    ...
    if (changed || chopt->verbosity == V_high) {
      ...
      describe_change(file_full_name, ch_status, old_usr, old_grp,
...
/* Change the owner and/or group of the specified FILES.
   ...
   Return true if successful.  */
extern bool chown_files(char **files, int bit_flags, uid_t uid, gid_t gid,
                        uid_t required_uid, gid_t required_gid,
                        struct Chown_option const *chopt) {
  bool ok = true;
