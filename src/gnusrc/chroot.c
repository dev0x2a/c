...
enum { GROUPS = UCHAR_MAX + 1, USERSPEC, SKIP_CHDIR };
...
static int setgroups(size_t size, gid_t const *list _GL_UNUSED) {
  if (size == 0) {
...
static int parse_additional_groups(char const *groups, GETGROUPS_T **pgids,
...
int main(int argc, char **argv) {
  ...
  if (uid_set(uid) && (!groups || gid_unset(gid))) {
    ...
    if ((pwd = getpwuid(uid))) {
      ...
      error(EXIT_CANCELED, errno, _("no group specified for unknown uid: %d"),
            (int)uid);
    }
  }
  ...
  if (groups && *groups) {
    ...
    if (ngroups <= 0) {
      if (!n_gids)
        error(EXIT_CANCELED, errno, _("failed to get supplemental groups"));
      /* else look-up outside the chroot worked, then go with those.  */
    } else {
      n_gids = ngroups;
      gids = in_gids;
