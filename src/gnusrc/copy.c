...
#ifndef HAVE_MKFIFO
static int rpl_mkfifo(char const *file, mode_t mode) {
  errno = ENOTSUP;
  return -1;
}
...
static inline int utimens_symlink(char const *file,
...
static bool create_hole(int fd, char const *name, bool punch_holes,
                        off_t size) {
...
/* Copy the regular file open on SRC_FD/SRC_NAME to DST_FD/DST_NAME,
   ...
   bytes read.  */
static bool sparse_copy(int src_fd, int dest_fd, char *buf, size_t buf_size,
      ...
      if (transition || last_chunk) {
        ...
        if (!prev_hole) {
          if (full_write(dest_fd, pbuf, psize) != psize) {
            error(0, errno, _("error writing %s"), quoteaf(dst_name));
            return false;
          }
        } else {
...
static bool write_zeros(int fd, off_t n_bytes) {
  static char *zeros;
...
/* Perform an efficient extent copy, if possible.  This avoids
   ...
   return false.  */
static bool extent_copy(int src_fd, int dest_fd, char *buf, size_t buf_size,
                        size_t hole_size, off_t src_total_size,
                        enum Sparse_type sparse_mode, char const *src_name,
                        char const *dst_name, bool *require_normal_copy) {
  ...
  *require_normal_copy = false;
  ...
  do {
    ...
    if (!ok) {
      ...
      error(0, errno, _("%s: failed to get extents info"), quotef(src_name));
      return false;
    }
    ...
    for (i = 0; i < scan.ei_count || empty_extent; i++) {
      ...
      if (ext_hole_size) {
        ...
        if ((empty_extent && sparse_mode == SPARSE_ALWAYS) ||
          ...
          if (!write_zeros(dest_fd, nzeros)) {
            error(0, errno, _("%s: write failed"), quotef(dst_name));
            goto fail;
          }
      ...
      if (0 && (scan.ext_info[i].ext_flags & FIEMAP_EXTENT_UNWRITTEN)) {
        ...
        if (!sparse_copy(src_fd, dest_fd, buf, buf_size,
                         sparse_mode == SPARSE_ALWAYS ? hole_size : 0, true,
                         src_name, dst_name, ext_len, &n_read, &read_hole))
          goto fail;
...
static bool _GL_ATTRIBUTE_PURE is_ancestor(const struct stat *sb,
                                           const struct dir_list *ancestors) {
  while (ancestors != 0) {
...
static bool errno_unsupported(int err) {
  return err == ENOTSUP || err == ENODATA;
}

#if USE_XATTR
static void copy_attr_error(struct error_context *ctx _GL_UNUSED,
                            char const *fmt, ...) {
...
static void copy_attr_allerror(struct error_context *ctx _GL_UNUSED,
                               char const *fmt, ...) {
  int err = errno;
  va_list ap;
...
static char const *copy_attr_quote(struct error_context *ctx _GL_UNUSED,
                                   char const *str) {
  return quoteaf(str);
}

static void copy_attr_free(struct error_context *ctx _GL_UNUSED,
...
/* Exclude SELinux extended attributes that are otherwise handled,
   ...
   Return zero to skip.  */
static int check_selinux_attr(const char *name, struct error_context *ctx) {
...
static bool copy_attr(char const *src_path, int src_fd, char const *dst_path,
                      int dst_fd, struct cp_options const *x) {
  int ret;
  ...
  struct error_context ctx = {.error = all_errors ? copy_attr_allerror
                                                  : copy_attr_error,
...
static bool copy_attr(char const *src_path _GL_UNUSED, int src_fd _GL_UNUSED,
                      char const *dst_path _GL_UNUSED, int dst_fd _GL_UNUSED,
...
static bool copy_dir(char const *src_name_in, char const *dst_name_in,
                     bool new_dst, const struct stat *src_sb,
  ...
  while (*namep != '\0') {
    ...
    ok &= copy_internal(src_name, dst_name, new_dst, src_sb, ancestors,
                        &non_command_line_options, false, &first_dir_created,
...
static void set_author(const char *dst_name, int dest_desc,
  ...
  file_t file =
      (dest_desc < 0 ? file_name_lookup(dst_name, 0, 0) : getdport(dest_desc));
...
static bool set_process_security_ctx(char const *src_name, char const *dst_name,
  ...
  if (x->preserve_security_context) {
    ...
    if (new_dst && defaultcon(dst_name, mode) < 0 &&
      ...
      error(0, errno, _("failed to set default file creation context for %s"),
            quoteaf(dst_name));
    }
  }
...
static bool set_file_security_ctx(char const *dst_name, bool process_local,
...
static bool is_probably_sparse(struct stat const *sb) {
  return (HAVE_STRUCT_STAT_ST_BLOCKS && S_ISREG(sb->st_mode) &&
...
static bool copy_reg(char const *src_name, char const *dst_name,
  ...
  source_desc =
      open(src_name, (O_RDONLY | O_BINARY |
  ...
  if (!SAME_INODE(*src_sb, src_open_sb)) {
    error(0, 0, _("skipping file %s, as it was replaced while being copied"),
          quoteaf(src_name));
    return_val = false;
    goto close_src_desc;
  }
  ...
  if (!*new_dst) {
    ...
    if ((x->set_security_context || x->preserve_security_context) &&
        0 <= dest_desc) {
      if (!set_file_security_ctx(dst_name, x->preserve_security_context, false,
                                 x)) {
        if (x->require_preserve_context) {
          return_val = false;
    ...
    if (dest_desc < 0 && x->unlink_dest_after_failed_open) {
      ...
      *new_dst = true;
      ...
      if (x->set_security_context) {
        if (!set_process_security_ctx(src_name, dst_name, dst_mode, *new_dst,
                                      x)) {
          return_val = false;
          goto close_src_desc;
        }
  ...
  open_with_O_CREAT:;
    ...
    dest_desc =
        open(dst_name, open_flags | O_EXCL, dst_mode & ~omitted_permissions);
    dest_errno = errno;
    ...
    if (dest_desc < 0 && dest_errno == EEXIST && !x->move_mode) {
      ...
      if (lstat(dst_name, &dangling_link_sb) == 0 &&
        ...
        if (x->open_dangling_dest_symlink) {
          dest_desc =
              open(dst_name, open_flags, dst_mode & ~omitted_permissions);
          dest_errno = errno;
        } else {
    ...
    if (dest_desc < 0 && dest_errno == EISDIR && *dst_name &&
  ...
  if (data_copy_required && x->reflink_mode) {
    ...
    if (clone_ok || x->reflink_mode == REFLINK_ALWAYS) {
      if (!clone_ok) {
        error(0, errno, _("failed to clone %s from %s"), quoteaf_n(0, dst_name),
  ...
  if (data_copy_required) {
    ...
    if (sparse_src) {
      ...
      if (extent_copy(source_desc, dest_desc, buf, buf_size, hole_size,
                      src_open_sb.st_size,
                      make_holes ? x->sparse_mode : SPARSE_NEVER, src_name,
                      dst_name, &normal_copy_required))
...
preserve_metadata:
  ...
  if (x->preserve_mode || x->move_mode) {
    ...
    if (omitted_permissions &&
      ...
      error(0, errno, _("preserving permissions for %s"), quoteaf(dst_name));
      if (x->require_preserve)
        return_val = false;
    }
  }
...
static bool same_file_ok(char const *src_name, struct stat const *src_sb,
  ...
  *return_now = false;
  ...
  if (x->dereference == DEREF_NEVER) {
    ...
    if (lstat(dst_name, &tmp_dst_sb) != 0 || lstat(src_name, &tmp_src_sb) != 0)
      return true;
  ...
  if (x->backup_type != no_backups) {
    if (!same_link) {
      ...
      if (!x->move_mode && x->dereference != DEREF_NEVER &&
          S_ISLNK(src_sb_link->st_mode) && !S_ISLNK(dst_sb_link->st_mode))
        return false;
  ...
  if (x->move_mode && S_ISLNK(src_sb->st_mode) && 1 < dst_sb_link->st_nlink) {
  ...
  if (x->unlink_dest_before_opening && S_ISLNK(dst_sb_link->st_mode))
...
static bool writable_destination(char const *file, mode_t mode) {
  return (S_ISLNK(mode) || can_write_any_file() || euidaccess(file, W_OK) == 0);
}

static bool overwrite_ok(struct cp_options const *x, char const *dst_name,
                         struct stat const *dst_sb) {
  if (!writable_destination(dst_name, dst_sb->st_mode)) {
    char perms[12]; /* "-rwxrwxrwx " ls-style modes. */
    strmode(dst_sb->st_mode, perms);
    perms[10] = '\0';
    fprintf(stderr, (x->move_mode || x->unlink_dest_before_opening ||
                     x->unlink_dest_after_failed_open)
                        ? _("%s: replace %s, overriding mode %04lo (%s)? ")
                        : _("%s: unwritable %s (mode %04lo, %s); try anyway? "),
            program_name, quoteaf(dst_name),
            (unsigned long int)(dst_sb->st_mode & CHMOD_MODE_BITS), &perms[1]);
  } else {
    fprintf(stderr, _("%s: overwrite %s? "), program_name, quoteaf(dst_name));
  }
...
extern void dest_info_init(struct cp_options *x) {
  x->dest_info = hash_initialize(DEST_INFO_INITIAL_CAPACITY, NULL, triple_hash,
...
extern void src_info_init(struct cp_options *x) {
  ...
  */
  x->src_info =
      hash_initialize(DEST_INFO_INITIAL_CAPACITY, NULL, triple_hash_no_name,
                      triple_compare, triple_free);
}
...
static bool abandon_move(const struct cp_options *x, char const *dst_name,
                         struct stat const *dst_sb) {
  assert(x->move_mode);
...
static bool create_hard_link(char const *src_name, char const *dst_name,
  ...
  bool link_failed =
      (linkat(AT_FDCWD, src_name, AT_FDCWD, dst_name, flags) != 0);
...
static bool copy_internal(char const *src_name, char const *dst_name,
  ...
  *copy_into_self = false;
  ...
  if (command_line_arg) {
    if (!S_ISDIR(src_sb.st_mode) && x->backup_type == no_backups &&
  ...
  if (!new_dst) {
    ...
    bool use_stat =
        ((S_ISREG(src_mode) ||
          (x->copy_as_regular && !(S_ISDIR(src_mode) || S_ISLNK(src_mode)))) &&
         !(x->move_mode || x->symbolic_link || x->hard_link ||
           x->backup_type != no_backups || x->unlink_dest_before_opening));
    if ((use_stat ? stat(dst_name, &dst_sb) : lstat(dst_name, &dst_sb)) != 0) {
      if (errno != ENOENT) {
    ...
    } else { /* Here, we know that dst_name exists, at least to the point
                that it is stat'able or lstat'able.  */
      bool return_now;

      have_dst_lstat = !use_stat;
      if (!same_file_ok(src_name, &src_sb, dst_name, &dst_sb, x, &return_now)) {
        error(0, 0, _("%s and %s are the same file"), quoteaf_n(0, src_name),
      ...
      if (x->move_mode) {
        ...
        if (!S_ISDIR(src_mode) && (x->interactive == I_ALWAYS_NO ||
                                   (x->interactive == I_ASK_USER &&
                                    !overwrite_ok(x, dst_name, &dst_sb))))
          return true;
      }
      ...
      if (x->backup_type != no_backups
        ...
        char *tmp_backup = find_backup_file_name(dst_name, x->backup_type);
        ...
        if (STREQ(tmp_backup, src_name)) {
          ...
          error(0, 0, fmt, quoteaf_n(0, dst_name), quoteaf_n(1, src_name));
          free(tmp_backup);
          return false;
        }
  ...
  if (command_line_arg && x->dest_info && !x->move_mode &&
    ...
    if (lstat_ok && S_ISLNK(dst_lstat_sb->st_mode) &&
        seen_file(x->dest_info, dst_name, dst_lstat_sb)) {
      error(0, 0, _("will not copy %s through just-created symlink %s"),
  ...
  if (x->move_mode) {
    ...
    if (errno == EINVAL) {
      ...
      error(0, 0, _("cannot move %s to a subdirectory of itself, %s"),
            quoteaf_n(0, top_level_src_name), quoteaf_n(1, top_level_dst_name));
    ...
    if (errno != EXDEV) {
      /* There are many ways this can happen due to a race condition.
         ...
         fail.  Etc.  */
      error(0, errno, _("cannot move %s to %s"), quoteaf_n(0, src_name),
            quoteaf_n(1, dst_name));
  ...
  omitted_permissions =
      (dst_mode_bits &
       (x->preserve_ownership ? S_IRWXG | S_IRWXO
                              : S_ISDIR(src_mode) ? S_IWGRP | S_IWOTH : 0));
  ...
  if (S_ISDIR(src_mode)) {
    ...
    if (is_ancestor(&src_sb, ancestors)) {
      error(0, 0, _("cannot copy cyclic symbolic link %s"), quoteaf(src_name));
      goto un_backup;
    }
    ...
    if (new_dst || !S_ISDIR(dst_sb.st_mode)) {
      ...
      if (lstat(dst_name, &dst_sb) != 0) {
        ...
        if (lchmod(dst_name, dst_mode | S_IRWXU) != 0) {
          error(0, errno, _("setting permissions for %s"), quoteaf(dst_name));
          goto un_backup;
        }
      }
    ...
    if (x->one_file_system && parent && parent->st_dev != src_sb.st_dev) {
      ...
      delayed_ok =
          copy_dir(src_name, dst_name, new_dst, &src_sb, dir, x,
  ...
  /* POSIX 2008 states that it is implementation-defined whether
    ...
    if (!create_hard_link(src_name, dst_name, false, false, dereference))
      goto un_backup;
  } else if (S_ISREG(src_mode) || (x->copy_as_regular && !S_ISLNK(src_mode))) {
    ...
    if (mknod(dst_name, src_mode & ~omitted_permissions, src_sb.st_rdev) != 0) {
      error(0, errno, _("cannot create special file %s"), quoteaf(dst_name));
      goto un_backup;
    }
    ...
    if (src_link_val == NULL) {
      error(0, errno, _("cannot read symbolic link %s"), quoteaf(src_name));
      goto un_backup;
    }
    ...
    if (x->preserve_ownership) {
      ...
      if (HAVE_LCHOWN && lchown(dst_name, src_sb.st_uid, src_sb.st_gid) != 0 &&
          !chown_failure_ok(x)) {
        error(0, errno, _("failed to preserve ownership for %s"), dst_name);
        goto un_backup;
      } else {
  ...
  if (x->preserve_timestamps) {
    ...
    if ((dest_is_symlink ? utimens_symlink(dst_name, timespec)
...
static bool _GL_ATTRIBUTE_PURE valid_options(const struct cp_options *co) {
  assert(co != NULL);
...
extern bool copy(char const *src_name, char const *dst_name,
  ...
  return copy_internal(src_name, dst_name, nonexistent_dst, NULL, NULL, options,
                       true, &first_dir_created_per_command_line_arg,
...
extern bool chown_failure_ok(struct cp_options const *x) {
