...
#define AUTHORS proper_name("Alex Deymo")

static struct option const long_options[] = {{GETOPT_HELP_OPTION_DECL},
...
static void launch_program(const char *prog_name, int prog_argc,
                           char **prog_argv) {
...
#define SINGLE_BINARY_PROGRAM(prog_name_str, main_name)                        \
  else if (STREQ(prog_name_str, prog_name)) prog_main =                        \
      single_binary_main_##main_name;
#include "coreutils.h"
#undef SINGLE_BINARY_PROGRAM
#endif
