...
static char const *const reflink_type_string[] = {"auto", "always", NULL};
...
static bool re_protect(char const *const_dst_name, size_t src_offset,
...
static bool make_dir_parents_private(char const *const_dir, size_t src_offset,
      ...
      if (missing_dir) {
        ...
        *new_dst = true;
        ...
        if (mkdir(dir, mkdir_mode) != 0) {
          error(0, errno, _("cannot make directory %s"), quoteaf(dir));
          return false;
        } else {
        ...
        if (lstat(dir, &stats)) {
          error(0, errno, _("failed to get attributes of %s"), quoteaf(dir));
          return false;
        }
        ...
        if ((stats.st_mode & S_IRWXU) != S_IRWXU) {
          ...
          if (lchmod(dir, stats.st_mode | S_IRWXU) != 0) {
            error(0, errno, _("setting permissions for %s"), quoteaf(dir));
            return false;
          }
        }
      } else if (!S_ISDIR(stats.st_mode)) {
        error(0, 0, _("%s exists but is not a directory"), quoteaf(dir));
        return false;
      } else
        *new_dst = false;
      *slash++ = '/';
...
static bool target_directory_operand(char const *file, struct stat *st,
                                     bool *new_dst) {
...
static bool do_copy(int n_files, char **file, const char *target_directory,
  ...
  if (no_target_directory) {
    if (target_directory)
      error(EXIT_FAILURE, 0, _("cannot combine --target-directory (-t) "
  ...
  } else /* !target_directory */
  {
    ...
    if (x->unlink_dest_after_failed_open && x->backup_type != no_backups &&
        STREQ(source, dest) && !new_dst && S_ISREG(sb.st_mode)) {
...
static void decode_preserve_arg(char const *arg, struct cp_options *x,
  ...
  do {
    ...
    /* process S.  */
    val = XARGMATCH(on_off ? "--preserve" : "--no-preserve", s, preserve_args,
                    preserve_vals);
    switch (val) {
    case PRESERVE_MODE:
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "abdfHilLnprst:uvxPRS:TZ", long_opts,
    ...
    case SPARSE_OPTION:
      x.sparse_mode =
          XARGMATCH("--sparse", optarg, sparse_type_string, sparse_type);
      break;
    ...
    case REFLINK_OPTION:
      if (optarg == NULL)
        ...
        x.reflink_mode =
            XARGMATCH("--reflink", optarg, reflink_type_string, reflink_type);
      break;
  ...
  if (x.preserve_security_context && (x.set_security_context || scontext))
    error(EXIT_FAILURE, 0, _("cannot set target context and preserve it"));

  if (x.require_preserve_context && !selinux_enabled)
    error(EXIT_FAILURE, 0, _("cannot preserve security context "
