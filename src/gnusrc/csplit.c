...
#define AUTHORS proper_name("Stuart Kemp"), proper_name("David MacKenzie")
...
enum { SUPPRESS_MATCHED_OPTION = CHAR_MAX + 1 };
...
static void set_input_file(const char *name) {
  if (!STREQ(name, "-") && fd_reopen(STDIN_FILENO, name, O_RDONLY, 0) < 0)
    error(EXIT_FAILURE, errno, _("cannot open %s for reading"), quoteaf(name));
}
...
static void write_to_file(uintmax_t last_line, bool ignore, int argnum) {
  struct cstring *line;
  ...
  if (first_line > last_line) {
    error(0, 0, _("%s: line number out of range"), quote(global_argv[argnum]));
    cleanup_fatal();
  }
...
static void handle_line_error(const struct control *,
                              uintmax_t) ATTRIBUTE_NORETURN;
static void handle_line_error(const struct control *p, uintmax_t repetition) {
  char buf[INT_BUFSIZE_BOUND(uintmax_t)];

  fprintf(stderr, _("%s: %s: line number out of range"), program_name,
          quote(umaxtostr(p->lines_required, buf)));
  if (repetition)
...
static void process_line_count(const struct control *p, uintmax_t repetition) {
  uintmax_t linenum;
...
static void regexp_error(struct control *p, uintmax_t repetition, bool ignore) {
  ...
  if (p->offset >= 0) {
    while (true) {
      ...
      if (line->str[line_len - 1] == '\n')
        line_len--;
      ret = re_search(&p->re_compiled, line->str, line_len, 0, line_len, NULL);
      if (ret == -2) {
...
static void delete_all_files(bool in_signal_handler) {
  unsigned int i;
...
static void check_for_offset(struct control *p, const char *str,
                             const char *num) {
  ...
  *end = '\0';
  ...
  if (str + 1 == end - 1 && *(str + 1) == '*')
    ...
    if (xstrtoumax(str + 1, NULL, 10, &val, "") != LONGINT_OK) {
      error(EXIT_FAILURE, 0, _("%s}: integer required between '{' and '}'"),
            quote(global_argv[argnum]));
    }
    p->repeat = val;
  }
...
static struct control *extract_regexp(int argnum, bool ignore,
                                      char const *str) {
  ...
  if (closing_delim == NULL)
    error(EXIT_FAILURE, 0, _("%s: closing delimiter '%c' missing"), str, delim);
...
static void parse_patterns(int argc, int start, char **argv) {
  ...
  for (i = start; i < argc; i++) {
    if (*argv[i] == '/' || *argv[i] == '%') {
      ...
      if (val == 0)
        error(EXIT_FAILURE, 0, _("%s: line number must be greater than zero"),
              argv[i]);
      if (val < last_val) {
        char buf[INT_BUFSIZE_BOUND(uintmax_t)];
...
static void check_format_conv_type(char *format, int flags) {
  ...
  default:
    if (isprint(ch))
      error(EXIT_FAILURE, 0, _("invalid conversion specifier in suffix: %c"),
            ch);
    else
      error(EXIT_FAILURE, 0,
...
static size_t max_out(char *format) {
  ...
  if (!percent)
    error(EXIT_FAILURE, 0, _("missing %% conversion specification in suffix"));
...
int main(int argc, char **argv) {
  ...
  size_t max_digit_string_len =
      (suffix ? max_out(suffix) : MAX(INT_STRLEN_BOUND(unsigned int), digits));
