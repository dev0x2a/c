...
enum { OUTPUT_DELIMITER_OPTION = CHAR_MAX + 1, COMPLEMENT_OPTION };
...
static inline void next_item(size_t *item_idx) {
  (*item_idx)++;
...
static inline bool print_kth(size_t k) { return current_rp->lo <= k; }

/* Return nonzero if K'th byte is the beginning of a range. */

static inline bool is_range_start_index(size_t k) {
...
static void cut_fields(FILE *stream) {
  ...
  while (1) {
    if (field_idx == 1 && buffer_first_field) {
      ...
      len = getndelim2(&field_1_buffer, &field_1_bufsize, 0, GETNLINE_NO_LIMIT,
    ...
    if (print_kth(field_idx)) {
      if (found_any_selected_field) {
        fwrite(output_delimiter_string, sizeof(char), output_delimiter_length,
               stdout);
      }
      found_any_selected_field = true;
...
static bool cut_file(char const *file) {
  FILE *stream;
