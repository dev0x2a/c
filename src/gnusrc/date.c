...
static char const *const time_spec_string[] = {
    /* Put "hours" and "minutes" first, since they aren't valid for
       --rfc-3339.  */
    "hours", "minutes", "date", "seconds", "ns", NULL};
...
enum { RFC_3339_OPTION = CHAR_MAX + 1 };
...
static bool batch_convert(const char *input_filename, const char *format,
                          timezone_t tz) {
  bool ok;
  FILE *in_stream;
  char *line;
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, short_options, long_options, NULL)) !=
    ...
    case RFC_3339_OPTION: {
      ...
      enum Time_spec i =
          XARGMATCH("--rfc-3339", optarg, time_spec_string + 2, time_spec + 2);
    ...
    case 'I': {
      ...
      enum Time_spec i =
          (optarg ? XARGMATCH("--iso-8601", optarg, time_spec_string, time_spec)
                  : TIME_SPEC_DATE);
  ...
  if (batch_file != NULL)
    ...
    if (!option_specified_date && !set_date) {
      if (optind < argc) {
        ...
        set_date = true;
        datestr = argv[optind];
        valid_date = posixtime(&when.tv_sec, datestr,
