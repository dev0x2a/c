...
enum {
  /* Compute a value that's bitwise disjoint from the union
     of all O_ values.  */
  v = ~(0 | O_APPEND | O_BINARY | O_CIO | O_DIRECT | O_DIRECTORY | O_DSYNC |
        O_NOATIME | O_NOCTTY | O_NOFOLLOW | O_NOLINKS | O_NONBLOCK | O_SYNC |
        O_TEXT),
...
static void alloc_ibuf(void) {
  ...
  if (!real_buf) {
    ...
    error(EXIT_FAILURE, 0,
          ...
          ibs, human_readable(input_blocksize, hbuf,
    ...
    if (!real_obuf) {
      ...
      error(EXIT_FAILURE, 0,
            _("memory exhausted by output buffer of size %" PRIuMAX
              " bytes (%s)"),
            obs, human_readable(output_blocksize, hbuf,
                                human_opts | human_base_1024, 1, 1));
    }
...
static inline bool multiple_bits_set(int i) { return MULTIPLE_BITS_SET(i); }

static bool abbreviation_lacks_prefix(char const *message) {
...
static void print_xfer_stats(xtime_t progress_time) {
  ...
  int stats_len =
      (abbreviation_lacks_prefix(si)
           ? fprintf(stderr, ngettext("%" PRIuMAX " byte copied, %g s, %s/s",
...
static void print_stats(void) {
  ...
  if (r_truncate != 0)
    fprintf(stderr, ngettext("%" PRIuMAX " truncated record\n",
...
static void cleanup(void) {
  ...
  if (close(STDOUT_FILENO) < 0)
    error(EXIT_FAILURE, errno, _("closing output file %s"),
          quoteaf(output_file));
}
...
static void ATTRIBUTE_NORETURN quit(int code) {
  finish_up();
  exit(code);
}
...
/* Note we're being careful here to only invalidate what
   we've read, so as not to dump any read ahead cache.  */
    ...
    if (output_offset != -1) {
      ...
      if (0 <= output_offset) {
#if HAVE_POSIX_FADVISE
        adv_ret = posix_fadvise(fd, output_offset, clen, POSIX_FADV_DONTNEED);
#else
        errno = ENOTSUP;
#endif
        output_offset += clen + pending;
...
static ssize_t iread(int fd, char *buf, size_t size) {
  ssize_t nread;
...
static size_t iwrite(int fd, char const *buf, size_t size) {
...
static bool _GL_ATTRIBUTE_PURE operand_matches(char const *str,
                                               char const *pattern,
                                               char delim) {
  while (*pattern)
    if (*str++ != *pattern++)
...
static uintmax_t parse_integer(const char *str, strtol_error *invalid) {
  uintmax_t n;
  char *suffix;
...
static bool _GL_ATTRIBUTE_PURE operand_is(char const *operand,
                                          char const *name) {
...
static void scanargs(int argc, char *const *argv) {
  ...
  for (i = optind; i < argc; i++) {
    ...
    if (operand_is(name, "if"))
      ...
      conversions_mask |=
          parse_symbols(val, conversions, false, N_("invalid conversion"));
      ...
      output_flags |=
          parse_symbols(val, flags, false, N_("invalid output flag"));
    else if (operand_is(name, "status"))
      status_level =
          parse_symbols(val, statuses, true, N_("invalid status level"));
    else {
  ...
  if (output_flags & (O_COUNT_BYTES | O_SKIP_BYTES)) {
    error(0, 0, "%s: %s", _("invalid output flag"),
          quote(output_flags & O_COUNT_BYTES ? "count_bytes" : "skip_bytes"));
    usage(EXIT_FAILURE);
  }
  ...
  warn_partial_read =
      (!(conversions_mask & C_TWOBUFS) && !(input_flags & O_FULLBLOCK) &&
       (skip_records || (0 < max_records && max_records < (uintmax_t)-1) ||
...
static void translate_buffer(char *buf, size_t nread) {
  char *cp;
  size_t i;
...
#define MT_SAME_POSITION(P, Q)                                                 \
  ((P).mt_resid == (Q).mt_resid && (P).mt_fileno == (Q).mt_fileno &&           \
...
static off_t skip_via_lseek(char const *filename, int fdesc, off_t offset,
  ...
  if (0 <= new_position && got_original_tape_position &&
      ioctl(fdesc, MTIOCGET, &s2) == 0 && MT_SAME_POSITION(s1, s2)) {
    if (status_level != STATUS_NONE)
...
static uintmax_t skip(int fdesc, char const *file, uintmax_t records,
                      size_t blocksize, size_t *bytes) {
...
static bool advance_input_after_read_error(size_t nbytes) {
  if (!input_seekable) {
...
static void copy_simple(char const *buf, size_t nread) {
...
static void copy_with_unblock(char const *buf, size_t nread) {
  size_t i;
  char c;
...
static void set_fd_flags(int fd, int add_flags, char const *name) {
  ...
  if (add_flags) {
    ...
    if (old_flags < 0)
      ...
      if (ok && old_flags != new_flags && fcntl(fd, F_SETFL, new_flags) == -1)
        ok = false;
    }
...
static int dd_copy(void) {
  ...
  if (skip_records != 0 || skip_bytes != 0) {
    uintmax_t us_bytes =
        input_offset + (skip_records * input_blocksize) + skip_bytes;
    uintmax_t us_blocks = skip(STDIN_FILENO, input_file, skip_records,
                               input_blocksize, &skip_bytes);
    ...
    if ((us_blocks || (!input_offset_overflow && us_bytes)) &&
        status_level != STATUS_NONE) {
      error(0, 0, _("%s: cannot skip to specified offset"), quotef(input_file));
    }
  }

  if (seek_records != 0 || seek_bytes != 0) {
    size_t bytes = seek_bytes;
    uintmax_t write_records = skip(STDOUT_FILENO, output_file, seek_records,
    ...
    if (S_ISREG(stdout_stat.st_mode) || S_TYPEISSHM(&stdout_stat)) {
      ...
      if (0 <= output_offset && stdout_stat.st_size < output_offset) {
        if (iftruncate(STDOUT_FILENO, output_offset) != 0) {
          error(0, errno, _("failed to truncate to %" PRIdMAX " bytes"
                            " in output file %s"),
...
int main(int argc, char **argv) {
  ...
  parse_long_options(argc, argv, PROGRAM_NAME, PACKAGE, Version, usage, AUTHORS,
                     (char const *)NULL);
  ...
  if (output_file == NULL) {
    ...
    int opts = (output_flags | (conversions_mask & C_NOCREAT ? 0 : O_CREAT) |
      ...
      if (iftruncate(STDOUT_FILENO, size) != 0) {
        ...
        if (fstat(STDOUT_FILENO, &stdout_stat) != 0)
          error(EXIT_FAILURE, errno, _("cannot fstat %s"),
                quoteaf(output_file));
        if (S_ISREG(stdout_stat.st_mode) || S_ISDIR(stdout_stat.st_mode) ||
  ...
  if (max_records == 0 && max_bytes == 0) {
    ...
    if (i_nocache && !invalidate_cache(STDIN_FILENO, 0)) {
      error(0, errno, _("failed to discard cache for: %s"), quotef(input_file));
      exit_status = EXIT_FAILURE;
    }
