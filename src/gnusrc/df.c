...
#define AUTHORS                                                                \
  proper_name_utf8("Torbjorn Granlund", "Torbj\303\266rn Granlund"),           \
      proper_name("David MacKenzie"), proper_name("Paul Eggert")
...
static struct field_data_t field_data[] =
    {[SOURCE_FIELD] = {SOURCE_FIELD, "source", OTHER_FLD, N_("Filesystem"), 14,
                       MBS_ALIGN_LEFT, false},

     [FSTYPE_FIELD] = {FSTYPE_FIELD, "fstype", OTHER_FLD, N_("Type"), 4,
                       MBS_ALIGN_LEFT, false},

     [SIZE_FIELD] = {SIZE_FIELD, "size", BLOCK_FLD, N_("blocks"), 5,
                     MBS_ALIGN_RIGHT, false},

     [USED_FIELD] = {USED_FIELD, "used", BLOCK_FLD, N_("Used"), 5,
                     MBS_ALIGN_RIGHT, false},

     [AVAIL_FIELD] = {AVAIL_FIELD, "avail", BLOCK_FLD, N_("Available"), 5,
                      MBS_ALIGN_RIGHT, false},

     [PCENT_FIELD] = {PCENT_FIELD, "pcent", BLOCK_FLD, N_("Use%"), 4,
                      MBS_ALIGN_RIGHT, false},

     [ITOTAL_FIELD] = {ITOTAL_FIELD, "itotal", INODE_FLD, N_("Inodes"), 5,
                       MBS_ALIGN_RIGHT, false},

     [IUSED_FIELD] = {IUSED_FIELD, "iused", INODE_FLD, N_("IUsed"), 5,
                      MBS_ALIGN_RIGHT, false},

     [IAVAIL_FIELD] = {IAVAIL_FIELD, "iavail", INODE_FLD, N_("IFree"), 5,
                       MBS_ALIGN_RIGHT, false},

     [IPCENT_FIELD] = {IPCENT_FIELD, "ipcent", INODE_FLD, N_("IUse%"), 4,
                       MBS_ALIGN_RIGHT, false},

     [TARGET_FIELD] = {TARGET_FIELD, "target", OTHER_FLD, N_("Mounted on"), 0,
                       MBS_ALIGN_LEFT, false},

     [FILE_FIELD] = {FILE_FIELD, "file", OTHER_FLD, N_("File"), 0,
                     MBS_ALIGN_LEFT, false}};
...
static char *hide_problematic_chars(char *cell) {
  char *p = cell;
  while (*p) {
...
static void alloc_field(int f, const char *c) {
  ncolumns++;
...
static bool _GL_ATTRIBUTE_PURE selected_fstype(const char *fstype) {
...
static bool _GL_ATTRIBUTE_PURE excluded_fstype(const char *fstype) {
  const struct fs_type_list *fsp;
...
static bool devlist_compare(void const *x, void const *y) {
...
static void devlist_free(void *p) { free(p); }
...
static void filter_mount_list(bool devices_only) {
  struct mount_entry *me;
  ...
  devlist_table = hash_initialize(mount_list_size, NULL, devlist_hash,
                                  devlist_compare, devlist_free);
  if (devlist_table == NULL)
    xalloc_die();
  ...
  for (me = mount_list; me;) {
    ...
    if ((me->me_remote && show_local_fs) || -1 == stat(me->me_mountdir, &buf)) {
      ...
      if (seen_dev) {
        bool target_nearer_root =
            strlen(seen_dev->me->me_mountdir) > strlen(me->me_mountdir);
        /* With bind mounts, prefer items nearer the root of the source */
        bool source_below_root =
            seen_dev->me->me_mntroot != NULL && me->me_mntroot != NULL &&
          ...
          /* Don't discard remote entries with different locations,
             ...
             a more accurate value in that case.  */
                   ...
                   || (!STREQ(seen_dev->me->me_devname, me->me_devname)
                       ...
                       && STREQ(me->me_mountdir, seen_dev->me->me_mountdir))) {
...
static struct mount_entry const *_GL_ATTRIBUTE_PURE me_for_dev(dev_t dev) {
...
static bool known_value(uintmax_t n) { return n < UINTMAX_MAX - 1; }
...
static char const *df_readable(bool negative, uintmax_t n, char *buf,
...
static void add_uint_with_neg_flag(uintmax_t *dest, bool *dest_neg,
...
static bool _GL_ATTRIBUTE_PURE has_uuid_suffix(char const *s) {
  size_t len = strlen(s);
...
static void get_dev(char const *disk, char const *mount_point, char const *file,
                    char const *stat_file, char const *fstype, bool me_dummy,
                    bool me_remote, const struct fs_usage *force_fsu,
                    bool process_all) {
  ...
  if (force_fsu)
    ...
    if (process_all && (errno == EACCES || errno == ENOENT)) {
      ...
      fsu.fsu_blocksize = fsu.fsu_blocks = fsu.fsu_bfree = fsu.fsu_bavail =
          fsu.fsu_files = fsu.fsu_ffree = UINTMAX_MAX;
    } else {
  ...
  for (col = 0; col < ncolumns; col++) {
    ...
    case ITOTAL_FIELD:
      cell = xstrdup(
          df_readable(false, v->total, buf, v->input_units, v->output_units));
      break;
    ...
    case IPCENT_FIELD: {
      ...
      if (!known_value(v->used) || !known_value(v->available))
        ;
      else if (!v->negate_used && v->used <= TYPE_MAXIMUM(uintmax_t) / 100 &&
...
static bool get_disk(char const *disk) {
  struct mount_entry const *me;
...
static void get_point(const char *point, const struct stat *statp) {
  struct stat disk_stats;
  struct mount_entry *me;
  struct mount_entry const *best_match = NULL;
  ...
  free(resolved);
  if (best_match && (stat(best_match->me_mountdir, &disk_stats) != 0 ||
  ...
  if (!best_match)
    for (me = mount_list; me; me = me->me_next) {
      ...
      if (statp->st_dev == me->me_dev && !STREQ(me->me_type, "lofs") &&
          (!best_match || best_match->me_dummy || !me->me_dummy)) {
...
static void get_entry(char const *name, struct stat const *statp) {
  if ((S_ISBLK(statp->st_mode) || S_ISCHR(statp->st_mode)) && get_disk(name))
    return;
...
static void add_fs_type(const char *fstype) {
...
static void add_excluded_fs_type(const char *fstype) {
  struct fs_type_list *fsp;
...
int main(int argc, char **argv) {
    ...
    for (fs_incl = fs_select_list; fs_incl; fs_incl = fs_incl->fs_next) {
      ...
      for (fs_excl = fs_exclude_list; fs_excl; fs_excl = fs_excl->fs_next) {
        if (STREQ(fs_incl->fs_name, fs_excl->fs_name)) {
          error(0, 0, _("file system type %s both selected and excluded"),
  ...
  mount_list = read_file_system_list(
      (fs_select_list != NULL || fs_exclude_list != NULL || print_type ||
       field_data[FSTYPE_FIELD].used || show_local_fs));
  ...
  if (mount_list == NULL) {
    ...
    int status = 0;
    if (!(optind < argc) ||
        (show_all_fs || show_local_fs || fs_select_list != NULL ||
         fs_exclude_list != NULL)) {
      status = EXIT_FAILURE;
    }
  ...
  if (file_systems_processed) {
    if (print_grand_total)
      get_dev("total", (field_data[SOURCE_FIELD].used ? "-" : "total"), NULL,
