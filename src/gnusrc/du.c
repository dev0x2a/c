...
static void show_date(const char *format, struct timespec when, timezone_t tz) {
  struct tm tm;
...
static void print_size(const struct duinfo *pdui, const char *string) {
...
static bool process_file(FTS *fts, FTSENT *ent) {
  ...
  if (info == FTS_DNR) {
    ...
    if (!excluded) {
      ...
      if (fts->fts_options & FTS_XDEV && FTS_ROOTLEVEL < ent->fts_level &&
          fts->fts_dev != sb->st_dev)
        excluded = true;
    }

    if (excluded || (!opt_count_all && (hash_all || (!S_ISDIR(sb->st_mode) &&
                                                     1 < sb->st_nlink)) &&
  ...
  duinfo_set(&dui, (apparent_size ? MAX(0, sb->st_size)
...
static bool du_files(char **files, int bit_flags) {
  ...
  if (*files) {
    ...
    while (1) {
      ...
      if (ent == NULL) {
        if (errno != 0) {
          error(0, errno, _("fts_read failed: %s"), quotef(fts->fts_path));
          ok = false;
        }
...
int main(int argc, char **argv) {
  ...
  if (opt_time) {
    ...
    if (*time_style == '+')
      ...
      switch (XARGMATCH("time style", time_style, time_style_args,
                        time_style_types)) {
  ...
  while (true) {
    ...
    if (!file_name[0]) {
      ...
      if (files_from == NULL)
        ...
        error(0, 0, "%s:%lu: %s", quotef(files_from), file_number,
