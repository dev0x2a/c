...
static int mpz_init_set_str(mpz_t z, char *s, int base) {
  return xstrtoimax(s, NULL, base, z, NULL) == LONGINT_OK ? 0 : -1;
}
static void mpz_add(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a + b;
  if ((val < a) != (b < 0))
...
static void mpz_sub(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a - b;
  if ((a < val) != (b < 0))
...
static void mpz_mul(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
  intmax_t val = a * b;
  if (!(a == 0 || b == 0 || ((val < 0) == ((a < 0) ^ (b < 0)) && val / a == b)))
    integer_overflow('*');
  r[0] = val;
}
static void mpz_tdiv_q(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
...
static void mpz_tdiv_r(mpz_t r, mpz_t a0, mpz_t b0) {
  intmax_t a = a0[0];
  intmax_t b = b0[0];
...
static char *mpz_get_str(char const *str, int base, mpz_t z) {
  (void)str;
  (void)base;
  char buf[INT_BUFSIZE_BOUND(intmax_t)];
  return xstrdup(imaxtostr(z[0], buf));
}
static int mpz_sgn(mpz_t z) { return z[0] < 0 ? -1 : 0 < z[0]; }
static int mpz_fits_ulong_p(mpz_t z) { return 0 <= z[0] && z[0] <= ULONG_MAX; }
static unsigned long int mpz_get_ui(mpz_t z) { return z[0]; }
static int mpz_out_str(FILE *stream, int base, mpz_t z) {
  (void)base;
  char buf[INT_BUFSIZE_BOUND(intmax_t)];
...
#define AUTHORS                                                                \
  proper_name("Mike Parker"), proper_name("James Youngman"),                   \
...
static void syntax_error(void) { error(EXPR_INVALID, 0, _("syntax error")); }

#if !HAVE_GMP
/* Report an integer overflow for operation OP and exit.  */
static void integer_overflow(char op) {
...
static bool _GL_ATTRIBUTE_PURE null(VALUE *v) {
  switch (v->type) {
  case integer:
...
static bool _GL_ATTRIBUTE_PURE looks_like_integer(char const *cp) {
  cp += (*cp == '-');
...
static bool nextarg(char const *str) {
  if (*args == NULL)
    return false;
  else {
...
static VALUE *docolon(VALUE *sv, VALUE *pv) {
  ...
  if (0 <= matchlen) {
    ...
    error(EXPR_FAILURE, (matchlen == -2 ? errno : EOVERFLOW),
...
static VALUE *eval7(bool evaluate) {
  VALUE *v;
...
static VALUE *eval6(bool evaluate) {
  VALUE *l;
  VALUE *r;
  VALUE *v;
  VALUE *i1;
...
static VALUE *eval5(bool evaluate) {
  VALUE *l;
  VALUE *r;
  VALUE *v;
...
static VALUE *eval4(bool evaluate) {
  VALUE *l;
  VALUE *r;
  ...
  while (1) {
    ...
    if (evaluate) {
      ...
      ((fxn == multiply ? mpz_mul : fxn == divide ? mpz_tdiv_q : mpz_tdiv_r)(
          l->u.i, l->u.i, r->u.i));
    }
    freev(r);
  }
...
static VALUE *eval3(bool evaluate) {
  VALUE *l;
  VALUE *r;
  enum { plus, minus } fxn;
...
static VALUE *eval2(bool evaluate) {
  VALUE *l;
    ...
    if (evaluate) {
      ...
      if (looks_like_integer(l->u.s) && looks_like_integer(r->u.s))
        ...
        if (errno) {
          ...
          error(EXPR_INVALID, 0, _("the strings compared were %s and %s"),
...
static VALUE *eval1(bool evaluate) {
  VALUE *l;
  VALUE *r;
...
static VALUE *eval(bool evaluate) {
  VALUE *l;
  VALUE *r;
