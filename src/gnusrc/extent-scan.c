...
static bool extent_need_sync(void) {
...
extern bool extent_scan_read(struct extent_scan *scan) {
  unsigned int si = 0;
  ...
  while (true) {
    ...
    scan->ei_count += fiemap->fm_mapped_extents;
    {
      ...
      scan->ext_info =
          xnrealloc(scan->ext_info, scan->ei_count, sizeof(struct extent_info));
...
#else
extern bool extent_scan_read(struct extent_scan *scan _GL_UNUSED) {
  scan->initial_scan_failed = true;
  errno = ENOTSUP;
  return false;
}
