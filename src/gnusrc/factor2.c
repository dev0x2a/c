...
static void mp_factor_insert(struct mp_factors *factors, mpz_t prime) {
  uint nfactors = factors->nfactors, *e = factors->e;
  mpz_t *p = factors->p;
  long i;
  /* Locate position for insert new or increment e.  */
  for (i = nfactors - 1; i >= 0; --i) {
...
static void factor_insert_refind(struct factors *factors, uintmax_t p, UINT i,
                                 UINT off) {
...
static bool _GL_ATTRIBUTE_CONST millerrabin(uintmax_t n, uintmax_t ni,
                                            uintmax_t b, uintmax_t q, UINT k,
                                            uintmax_t one) {
...
static void factor_using_pollard_rho2(uintmax_t n1, uintmax_t n0, uint a,
                                      struct factors *factors) {
...
#if HAVE_GMP
static void mp_factor_using_pollard_rho(mpz_t n, uint a,
                                        struct mp_factors *factors) {
  mpz_t x, z, y, P, t, t2;
  ...
  unsigned long long int k = 1, l = 1;
...
static bool factor_using_squfof(uintmax_t n1, uintmax_t n0,
                                struct factors *factors) {
...
static struct lbuf_ { char *buf, *end; } lbuf;
