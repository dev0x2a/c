...
#define AUTHORS proper_name("David MacKenzie"), proper_name("Jim Meyering")
...
enum { PRESUME_INPUT_PIPE_OPTION = CHAR_MAX + 1 };
...
static off_t elseek(int fd, off_t offset, int whence, char const *filename) {
  ...
  if (new_offset < 0)
    error(0, errno,
          _(whence == SEEK_SET ? N_("%s: cannot seek to offset %s")
                               : N_("%s: cannot seek to relative offset %s")),
          quotef(filename), offtostr(offset, buf));
...
static bool elide_tail_bytes_pipe(const char *filename, int fd,
                                  uintmax_t n_elide_0, off_t current_pos) {
...
static bool elide_tail_bytes_file(const char *filename, int fd,
                                  uintmax_t n_elide, struct stat const *st,
                                  off_t current_pos) {
...
static bool elide_tail_lines_pipe(const char *filename, int fd,
                                  uintmax_t n_elide, off_t current_pos) {
  struct linebuffer {
    char buffer[BUFSIZ];
    size_t nbytes;
    size_t nlines;
...
static bool elide_tail_lines_seekable(const char *pretty_filename, int fd,
                                      uintmax_t n_lines, off_t start_pos,
                                      off_t size) {
  char buffer[BUFSIZ];
  size_t bytes_read;
  off_t pos = size;
...
static bool elide_tail_lines_file(const char *filename, int fd,
                                  uintmax_t n_elide, struct stat const *st,
                                  off_t current_pos) {
  off_t size = st->st_size;
  if (presume_input_pipe || size <= ST_BLKSIZE(*st))
...
static bool head_bytes(const char *filename, int fd, uintmax_t bytes_to_write) {
  char buffer[BUFSIZ];
...
static bool head_lines(const char *filename, int fd, uintmax_t lines_to_write) {
  char buffer[BUFSIZ];
...
static bool head(const char *filename, int fd, uintmax_t n_units,
                 bool count_lines, bool elide_from_end) {
  if (print_headers)
  ...
  if (elide_from_end) {
    ...
    struct stat st;
    if (fstat(fd, &st) != 0) {
      error(0, errno, _("cannot fstat %s"), quoteaf(filename));
      return false;
    }
...
static bool head_file(const char *filename, uintmax_t n_units, bool count_lines,
                      bool elide_from_end) {
  int fd;
  bool ok;
...
static uintmax_t string_to_integer(bool count_lines, const char *n_string) {
