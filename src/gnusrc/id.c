...
#define AUTHORS proper_name("Arnold Robbins"), proper_name("David MacKenzie")
...
static struct option const longopts[] = {
    ...
    {"groups", no_argument, NULL, 'G'},  {"name", no_argument, NULL, 'n'},
    {"real", no_argument, NULL, 'r'},    {"user", no_argument, NULL, 'u'},
    {"zero", no_argument, NULL, 'z'},    {GETOPT_HELP_OPTION_DECL},
...
/* politely decline if we're not on a SELinux/SMACK-enabled kernel. */
      ...
      if (!selinux_enabled && !smack_enabled)
        error(EXIT_FAILURE, 0, _("--context (-Z) works only on "
  ...
  if (default_format && opt_zero)
    error(EXIT_FAILURE, 0, _("option --zero not permitted in default format"));
...
static void print_user(uid_t uid) {
  ...
  if (use_name) {
    pwd = getpwuid(uid);
    if (pwd == NULL) {
      error(0, 0, _("cannot find name for user ID %s"), uidtostr(uid));
      ok = false;
    }
  }
...
static void print_full_info(const char *username) {
...

  {
    ...
    if (n_groups < 0) {
      if (username)
        error(0, errno, _("failed to get groups for user %s"), quote(username));
      else
