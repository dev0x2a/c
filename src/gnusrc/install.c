...
enum { PRESERVE_CONTEXT_OPTION = CHAR_MAX + 1, STRIP_PROGRAM_OPTION };
...
static bool need_copy(const char *src_name, const char *dest_name,
...
static void setdefaultfilecon(char const *file) {
  ...
  if (lsetfilecon(file, scontext) < 0 && errno != ENOTSUP)
    error(0, errno, _("warning: %s: failed to change context to %s"),
...
#else
static void setdefaultfilecon(char const *file) { (void)file; }
#endif
...
static bool target_directory_operand(char const *file) {
  ...
  if (is_a_dir < looks_like_a_dir)
    error(EXIT_FAILURE, err, _("target %s is not a directory"), quoteaf(file));
  return is_a_dir;
}
...
static int process_dir(char *dir, struct savewd *wd, void *options) {
  ...
  int ret = (make_dir_parents(dir, wd, make_ancestor, options, dir_mode,
                              announce_mkdir, dir_mode_bits, owner_id, group_id,
                              false)
                 ? EXIT_SUCCESS
                 : EXIT_FAILURE);
...
static bool change_timestamps(struct stat const *src_sb, char const *dest) {
...
static bool strip(char const *name) {
  int status;
  bool ok = false;
  pid_t pid = fork();
...
static bool mkancesdirs_safe_wd(char const *from, char *to,
                                struct cp_options *x, bool save_always) {
  ...
  if (save_working_directory) {
    ...
    if (restore_result < 0 && status == EXIT_SUCCESS) {
      error(0, restore_errno, _("cannot create directory %s"), quoteaf(to));
      return false;
    }
  }
