    ...
    if (!issued_disorder_warning[whatfile - 1]) {
      ...
      if (keycmp(prev, current, join_field, join_field) > 0) {
        ...
        error((check_input_order == CHECK_ORDER_ENABLED ? EXIT_FAILURE : 0), 0,
              _("%s:%" PRIuMAX ": is not sorted: %.*s"), g_names[whatfile - 1],
...
static bool getseq(FILE *fp, struct seq *seq, int whichfile) {
...
static bool advance_seq(FILE *fp, struct seq *seq, bool first, int whichfile) {
  if (first)
    seq->count = 0;
...
static void join(FILE *fp1, FILE *fp2) {
...
static void decode_field_spec(const char *s, int *file_index,
                              size_t *field_index) {
  ...
  default:
    error(EXIT_FAILURE, 0, _("invalid file number in field spec: %s"),
          quote(s));
...
static void set_join_field(size_t *var, size_t val) {
  if (*var != SIZE_MAX && *var != val) {
    ...
    error(EXIT_FAILURE, 0, _("incompatible join fields %lu, %lu"), var1, val1);
  }
  *var = val;
}
  ...
  while ((optc = getopt_long(argc, argv, "-a:e:i1:2:j:o:t:v:z", longopts,
    ...
    case 'a': {
      ...
      if (xstrtoul(optarg, NULL, 10, &val, "") != LONGINT_OK ||
          (val != 1 && val != 2))
        error(EXIT_FAILURE, 0, _("invalid field number: %s"), quote(optarg));
      if (val == 1)
    ...
    case 't': {
      ...
      if (!newtab)
        ...
        if (STREQ(optarg, "\\0"))
          ...
          error(EXIT_FAILURE, 0, _("multi-character tab %s"), quote(optarg));
      }
      if (0 <= tab && tab != newtab)
