...
#define strsignal(signum)                                                      \
  (0 <= (signum) && (signum) <= SIGNUM_BOUND ? sys_siglist[signum] : 0)
#endif
#ifndef strsignal
...
static struct option const long_options[] = {
    {"list", no_argument, NULL, 'l'},  {"signal", required_argument, NULL, 's'},
    {"table", no_argument, NULL, 't'}, {GETOPT_HELP_OPTION_DECL},
...
static int list_signals(bool table, char *const *argv) {
  int signum;
...
no_more_options:
  ...
  return (list ? list_signals(table, optind < argc ? argv + optind : NULL)
