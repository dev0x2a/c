...
#define AUTHORS proper_name("Mike Parker"), proper_name("David MacKenzie")
...
static bool errno_nonexisting(int err) {
...
static bool do_link(const char *source, const char *dest) {
  ...
  if (dest_lstat_ok && dest_set != NULL &&
  ...
  if ((remove_existing_files
    ...
    error(0, 0, _("%s and %s are the same file"), quoteaf_n(0, source),
          quoteaf_n(1, dest));
    return false;
  }
    ...
    if (backup_type != no_backups) {
      ...
      if (rename(dest, dest_backup) != 0) {
        ...
        if (rename_errno != ENOENT) {
          error(0, rename_errno, _("cannot backup %s"), quoteaf(dest));
          return false;
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  if (no_target_directory) {
    if (target_directory)
      error(EXIT_FAILURE, 0, _("cannot combine --target-directory "
  ...
  if (target_directory) {
    ...
    {
      dest_set = hash_initialize(DEST_INFO_INITIAL_CAPACITY, NULL, triple_hash,
