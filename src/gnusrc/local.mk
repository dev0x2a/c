...
##Copyright(C) 1990 - 2016 Free Software Foundation,
    Inc.
...
#FIXME : once lib / and gnulib - tests / are also converted,                   \
    hoist to Makefile.am
...
#@kgit = 'https://git.kernel.org/cgit/linux/kernel/git';                       \
    #wget - q $$kgit / torvalds /                                              \
    linux.git / plain / include / uapi / linux / magic.h #- O $ @
...
src $(AM_V_at) $(PERL) $(srcdir) / src / extract
        - magic-- local $(srcdir) / src / stat.c
    > $ @t $(AM_V_at) chmod a - w $ @t $(AM_V_at) mv $ @t $ @

          BUILT_SOURCES += src / fs.h src / fs.h : src / stat.c src / extract
                           - magic $(AM_V_GEN) rm - f $ @$(AM_V_at) $ {
  MKDIR_P
}
src $(AM_V_at) $(PERL) $(srcdir) / src / extract
        - magic $(srcdir) / src / stat.c
    > $ @t $(AM_V_at) chmod a - w $ @t $(AM_V_at) mv $ @t $ @

          BUILT_SOURCES += src / version.c src
                               / version.c : Makefile $(AM_V_GEN) rm
                           - f $ @$(AM_V_at) $ {
  MKDIR_P
}
src $(AM_V_at) printf '#include <config.h>\n' > $ @t $(AM_V_at) printf
    'char const *Version = "$(PACKAGE_VERSION)";\n' >>
    $ @t $(AM_V_at) chmod a - w $ @t $(AM_V_at) mv $ @t $ @

        BUILT_SOURCES += src / version.h src
                             / version.h : Makefile $(AM_V_GEN) rm
                         - f $ @$(AM_V_at) $ {
  MKDIR_P
}
src $(AM_V_at) printf 'extern char const *Version;\n' >
    $ @t $(AM_V_at) chmod a - w $ @t $(AM_V_at) mv $ @t $ @
...
#SINGLE_BINARY_PROGRAM("[", _)
        DISTCLEANFILES += src / coreutils.h src
                              / coreutils.h : Makefile $(AM_V_GEN) rm
                          - f $ @$(AM_V_at) $ {
  MKDIR_P
} src
	$(AM_V_at)for prog in x $(single_binary_progs);
do
...
echo "SINGLE_BINARY_PROGRAM(\"$$prog\", $$main)";
    ...
    sort > $ @t $(AM_V_at) chmod a - w $ @t $(AM_V_at) mv $ @t $ @

               DISTCLEANFILES += src / version.c src
                                 / version.h MAINTAINERCLEANFILES
    += $(BUILT_SOURCES)

        all_programs = $(bin_PROGRAMS) $(bin_SCRIPTS) $(EXTRA_PROGRAMS)

            pm = progs - makefile pr =
                     progs - readme
#Ensure that the list of programs in README matches the list
#of programs we can build.
                             check
                             - local : check - README check - duplicate - no -
                                       install.PHONY : check - README check -
                                       README : $(AM_V_GEN) rm - rf
                                                $(pr) $(pm) $(AM_V_at) echo
                                                $(all_programs) |
                         tr - s ' ' '\n' |
                         sed - e 's,$(EXEEXT)$$,,' - e 's,^src/,,' - e
                             's/^ginstall$$/install/' |
                         sed / libstdbuf / d | $(ASSORT) - u > $(pm) &&
                     sed -
                             n '/^The programs .* are:/,/^[a-zA-Z]/p' $(
                                 top_srcdir) /
                                 README
                         | sed - n '/^   */s///p' |
                         tr - s ' ' '\n' > $(pr) $(AM_V_at) diff $(pm) $(pr) &&
                     rm -
                         rf $(pr) $(pm)
...
#by default.
                             .PHONY : check
                         - duplicate - no - install check - duplicate - no
                         - install : src / tr $(AM_V_GEN) test - z
                         "`echo '$(EXTRA_PROGRAMS)' | tr ' ' '\n' | uniq -d`"

#Use the just - built 'ginstall', when not cross - compiling.
                         if CROSS_COMPILING cu_install_program =
                         @INSTALL_PROGRAM
    @ else cu_install_program = src / ginstall endif INSTALL_PROGRAM =
                                    $(cu_install_program)
