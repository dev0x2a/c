...
static size_t quote_name(FILE *out, const char *name,
                         struct quoting_options const *options, size_t *width);
...
#define FILE_OR_LINK_MODE(File) \
  ((color_symlink_as_referent && (File)->linkok) ? (File)->linkmode \
                                                 : (File)->stat.st_mode)
...
static char const *const format_args[] = {"verbose",       "long",   "commas",
...
static enum sort_type const sort_types[] = {sort_none, sort_time, sort_size,
                                            sort_extension, sort_version};
ARGMATCH_VERIFY(sort_args, sort_types);
static char const *const time_args[] = {"atime", "access", "use",
                                        "ctime", "status", NULL};
...
static char const *const color_args[] = {
    ...
    "always", "yes",  "force", "never",  "no",
    "none",   "auto", "tty",   "if-tty", NULL};
static enum color_type const color_types[] = {
    color_always, color_always, color_always, color_never, color_never,
    color_never,  color_if_tty, color_if_tty, color_if_tty};
ARGMATCH_VERIFY(color_args, color_types);

/* Information about filling a column.  */
struct column_info {
  bool valid_len;
  size_t line_len, *col_arr;
};
...
#define DIRED_INDENT() \
  do { \
    if (dired) DIRED_FPUTS_LITERAL("  ", stdout); \
  } while (0)
...
#define PUSH_CURRENT_DIRED_POS(obs) \
  do { \
    if (dired) obstack_grow(obs, &dired_pos, sizeof(dired_pos)); \
  } while (0)
...
static size_t abmon_init(void)
{
  ...
  do {
    ...
    for (int i=0; i<12; ++i) {
      ...
      size_t req = mbsalign(nl_langinfo(ABMON_1+i), abmon[i],
                            sizeof(abmon[i]), &width, MBS_ALIGN_LEFT, 0);
...
static bool dev_ino_compare(void const *x, void const *y)
{
...
{
  struct dev_ino *ent, *ent_from_table;
  bool found_match;
...
static bool is_colored(enum indicator_no type)
{
  ...
  return !(len==0 || (len == 1 && STRNCMP_LIT(s, "0")==0) ||
...
int main(int argc, char *argv[])
{
  ...
  if (recursive) {
    active_dir_set = hash_initialize(INITIAL_TABLE_SIZE, NULL, dev_ino_hash,
...
static bool set_line_length(char const *spec)
{
  uintmax_t val;
...
static int decode_switches(int argc, char **argv)
{
  ...
  while (true) {
    ...
    case 'S':
      ...
      break;
    case 'T':
      tabsize = xnumtoumax(optarg, 0, 0, SIZE_MAX, "", _("invalid tab size"),
                           LS_FAILURE); break;
    case 'U':
      sort_type = sort_none;
      sort_type_specified = true;
    ...
    case COLOR_OPTION: {
      ...
      if (optarg)
        ...
        i = color_always;
      print_with_color =
          (i == color_always || (i == color_if_tty && isatty(STDOUT_FILENO)));
      if (print_with_color) {
    ...
    case INDICATOR_STYLE_OPTION:
      indicator_style = XARGMATCH("--indicator-style", optarg,
                                  indicator_style_args, indicator_style_types);
      break;
    case QUOTING_STYLE_OPTION:
      set_quoting_style(NULL,
                        XARGMATCH("--quoting-style", optarg, quoting_style_args,
                                  quoting_style_vals)); break;
...
static bool get_funky_string(
    char **dest, const char **src, bool equals_end, size_t *output_count)
{
  ...
  while (true) {
    ...
    case PS_START: /* First label character */
      ...
      case '*':
        ...
        ext->ext.string = buf;
        state =
            (get_funky_string(&buf, &p, true, &ext->ext.len) ? PS_4 : PS_FAIL);
        break;
      case '\0':
        state = PS_DONE; /* Done! */
        goto done;
      ...
      if (*(p++) == '=') { /* It *should* be...  */
        for (ind_no = 0; indicator_name[ind_no] != NULL; ++ind_no) {
          if (STREQ(label, indicator_name[ind_no])) {
            ...
            state =
                (get_funky_string(&buf, &p, false, &color_indicator[ind_no].len)
                     ? PS_START
                     : PS_FAIL);
            break;
          }
    ...
    case PS_4: /* Equal sign after *.ext */
      if (*(p++) == '=') {
        ext->seq.string = buf;
        state = (get_funky_string(&buf, &p, false, &ext->seq.len) ? PS_START
                                                                  : PS_FAIL);
      } else
        state = PS_FAIL;
      break;
    case PS_FAIL:goto done;
    default:abort();
    }
  }
done:
  if (state == PS_FAIL) {
    struct color_ext_type *e, *e2;
...
static void file_failure(bool serious, char const *message, char const *file)
{
...
static void queue_directory(
    char const *name, char const *realname, bool command_line_arg)
{
    ...
    char const *name, char const *realname, bool command_line_arg)
{
  ...
  if (LOOP_DETECT) {
    ...
    if ((0 <= fd ? fstat(fd, &dir_stat) : stat(name, &dir_stat)) < 0) {
...
#endif
        total_blocks +=
            gobble_file(next->d_name, type, RELIABLE_D_INO(next), false, name);
  ...
  if (format==long_format || print_block_size) {
    ...
    DIRED_PUTCHAR(' ');
    p = human_readable(total_blocks, buf, human_output_opts, ST_NBLOCKSIZE,
                       output_block_size);
    DIRED_FPUTS(p, stdout, strlen(p));
    DIRED_PUTCHAR('\n');
  }
...
static bool patterns_match(
    struct ignore_pattern const *patterns, char const *file)
{
...
#else
static bool has_capability(char const *name _GL_UNUSED)
{
  errno = ENOTSUP;
  return false;
}
...
static int getfilecon_cache(char const *file, struct fileinfo *f, bool deref)
{
...
#endif
    r = (deref ? getfilecon(file, &f->scontext)
...
static bool has_capability_cache(char const *file, struct fileinfo *f)
{
  /* st_dev of the most recently processed device for which we've
...

  {
    ...
    case DEREF_COMMAND_LINE_SYMLINK_TO_DIR:
      if (command_line_arg) {
        ...
        need_lstat = (err < 0 ? errno == ENOENT : !S_ISDIR(f->stat.st_mode));
        if (!need_lstat)
          break;
    ...
    if ((type == normal || S_ISREG(f->stat.st_mode)) && print_with_color &&
        is_colored(C_CAP))
    ...
    if (S_ISLNK(f->stat.st_mode) &&
      ...
      if (linkname && (file_type <= indicator_style || check_symlink_color) &&
    ...
    if (format == long_format) {
      ...
      if (S_ISCHR(f->stat.st_mode) || S_ISBLK(f->stat.st_mode)) {
        ...
        int len = mbswidth(human_readable(size, buf, file_human_output_opts, 1,
...
static int xstrcoll(char const *a, char const *b) {
  ...
  diff = strcoll(a, b);
  if (errno) {
    error(0, errno, _("cannot compare file names %s and %s"), quote_n(0, a),
          quote_n(1, b));
...
static inline int cmp_ctime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_ctime(&b->stat), get_stat_ctime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

static inline int cmp_mtime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_mtime(&b->stat), get_stat_mtime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

static inline int cmp_atime(struct fileinfo const *a, struct fileinfo const *b,
                            int (*cmp)(char const *, char const *)) {
  int diff = timespec_cmp(get_stat_atime(&b->stat), get_stat_atime(&a->stat));
  return diff ? diff : cmp(a->name, b->name);
}

static inline int cmp_size(struct fileinfo const *a, struct fileinfo const *b,
                           int (*cmp)(char const *, char const *)) {
  int diff = longdiff(b->stat.st_size, a->stat.st_size);
  return diff ? diff : cmp(a->name, b->name);
}

static inline int cmp_name(struct fileinfo const *a, struct fileinfo const *b,
                           int (*cmp)(char const *, char const *)) {
...
static inline int cmp_extension(struct fileinfo const *a,
...
static inline int cmp_version(struct fileinfo const *a,
                              struct fileinfo const *b) {
...
static size_t align_nstrftime(char *buf, size_t size, char const *fmt,
  ...
  const char *pb;
  if (required_mon_width && (pb = strstr(fmt, "%b")) && 0 <= tm->tm_mon &&
      tm->tm_mon <= 11) {
...
static int long_time_expected_width(void) {
  ...
  if (width < 0) {
    ...
    if (localtime_rz(localtz, &epoch, &tm)) {
      size_t len = align_nstrftime(buf, sizeof buf, long_time_format[0], &tm,
                                   localtz, 0);
      if (len != 0)
...
static void format_user_or_group(char const *name, unsigned long int id,
                                 int width) {
  size_t len;
...
static void format_user(uid_t u, int width, bool stat_ok) {
  format_user_or_group(!stat_ok ? "?" : (numeric_ids ? NULL : getuser(u)), u,
                       width);
}
...
static void format_group(gid_t g, int width, bool stat_ok) {
  format_user_or_group(!stat_ok ? "?" : (numeric_ids ? NULL : getgroup(g)), g,
                       width);
}

/* Return the number of columns that format_user_or_group will print.  */

static int format_user_or_group_width(char const *name, unsigned long int id) {
  if (name) {
...
static void print_long_format(const struct fileinfo *f) {
  char modebuf[12];
  char buf[LONGEST_HUMAN_READABLE + 1     /* inode */
  ...
  if (print_block_size) {
    ...
    char const *blocks =
        (!f->stat_ok ? "?" : human_readable(ST_NBLOCKS(f->stat), hbuf,
  ...
  if (f->stat_ok && (S_ISCHR(f->stat.st_mode) || S_ISBLK(f->stat.st_mode))) {
    ...
    sprintf(p, "%*s, %*s ", major_device_number_width + MAX(0, blanks_width),
    ...
    char hbuf[LONGEST_HUMAN_READABLE + 1];
    char const *size =
        (!f->stat_ok ? "?" : human_readable(unsigned_file_size(f->stat.st_size),
  ...
  *p = '\1';
  ...
  if (f->stat_ok && localtime_rz(localtz, &when_timespec.tv_sec, &when_local)) {
    ...
    s = align_nstrftime(p, TIME_STAMP_LEN_MAXIMUM + 1, fmt, &when_local,
  ...
  if (s || !*p) {
    ...
    *p = '\0';
    ...
    sprintf(p, "%*s ", long_time_expected_width(),
            (!f->stat_ok ? "?" : timetostr(when_timespec.tv_sec, hbuf)));
...
static size_t print_name_with_quoting(const struct fileinfo *f,
  ...
  bool used_color_this_time =
      (print_with_color &&
       (print_color_indicator(f, symlink_target) || is_colored(C_NORM)));
...
static size_t print_file_name_and_frills(const struct fileinfo *f,
  ...
  if (print_block_size)
    printf("%*s ", format == with_commas ? 0 : block_size_width,
           !f->stat_ok ? "?" : human_readable(ST_NBLOCKS(f->stat), buf,
                                              human_output_opts, ST_NBLOCKSIZE,
                                              output_block_size));
...
static size_t length_of_file_name_and_frills(const struct fileinfo *f) {
  ...
  if (print_block_size)
    len +=
        1 +
        (format == with_commas
             ? strlen(!f->stat_ok ? "?" : human_readable(ST_NBLOCKS(f->stat),
