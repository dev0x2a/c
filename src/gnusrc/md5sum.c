...
#define AUTHORS                                                                \
  proper_name("Ulrich Drepper"), proper_name("Scott Miller"),                  \
...
"),
           program_name, DIGEST_TYPE_STRING, DIGEST_BITS);
...
static bool bsd_split_3(char *s, size_t s_len, unsigned char **hex_digest,
...
static bool split_3(char *s, size_t s_len, unsigned char **hex_digest,
    ...
    if (s[i + algo_name_len] == '(') {
      *binary = 0;
      return bsd_split_3(s + i + algo_name_len + 1,
                         s_len - (i + algo_name_len + 1), hex_digest, file_name,
                         escaped_filename);
    }
  }
...
static bool _GL_ATTRIBUTE_PURE hex_digits(unsigned char const *s) {
  unsigned int i;
...
static void print_filename(char const *file, bool escape) {
  if (!escape) {
    fputs(file, stdout);
    return;
  }
...
static bool digest_file(const char *filename, int *binary,
...
static bool digest_check(const char *checkfile_name) {
  ...
  do {
    ...
    if (!(split_3(line, line_length, &hex_digest, &binary, &filename) &&
          !(is_stdin && STREQ(filename, "-")) && hex_digits(hex_digest))) {
  ...
  if (!properly_formatted_lines) {
    ...
    if (!status_only) {
      ...
      if (ignore_missing && !matched_checksums)
        error(0, 0, _("%s: no file was verified"), quotef(checkfile_name));
    }
  }
