...
static struct option const longopts[] = {{GETOPT_SELINUX_CONTEXT_OPTION_DECL},
...
int main(int argc, char **argv) {
  ...
  if (specified_mode) {
    ...
    if (newmode & ~S_IRWXUGO)
      error(EXIT_FAILURE, 0, _("mode must specify only file permission bits"));
  }
  ...
  if (expected_operands < argc - optind) {
    error(0, 0, _("extra operand %s"), quote(argv[optind + expected_operands]));
  ...
  block_or_character : {
    ...
    if (xstrtoumax(s_major, NULL, 0, &i_major, NULL) != LONGINT_OK ||
      ...
      error(EXIT_FAILURE, 0, _("invalid major device number %s"),
            quote(s_major));

    if (xstrtoumax(s_minor, NULL, 0, &i_minor, NULL) != LONGINT_OK ||
        i_minor != (minor_t)i_minor)
      error(EXIT_FAILURE, 0, _("invalid minor device number %s"),
            quote(s_minor));
