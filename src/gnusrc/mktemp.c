...
int main(int argc, char **argv) {
  ...
  if (suffix) {
    ...
    if (!len || template[len - 1] != 'X') {
      error(EXIT_FAILURE, 0, _("with --suffix, template %s must end in X"),
            quote(template));
    }
  ...
  if (suffix_len && last_component(suffix) != suffix) {
    error(EXIT_FAILURE, 0, _("invalid suffix %s, contains directory separator"),
          quote(suffix));
  }
  ...
  if (use_dest_dir) {
    if (deprecated_t_option) {
      ...
      if (IS_ABSOLUTE_FILE_NAME(template))
        error(EXIT_FAILURE, 0, _("invalid template, %s; with --tmpdir,"
