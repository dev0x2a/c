...
int main(int argc, char **argv) {
  ...
  if (redirecting_stdout || (redirecting_stderr && stdout_is_closed)) {
    ...
    out_fd = (redirecting_stdout ? fd_reopen(STDOUT_FILENO, file, flags, mode)
    ...
    if (out_fd < 0) {
      ...
      if (home) {
        ...
        out_fd =
            (redirecting_stdout ? fd_reopen(STDOUT_FILENO, in_home, flags, mode)
      ...
      if (out_fd < 0) {
        ...
        if (in_home)
          error(0, saved_errno2, _("failed to open %s"), quoteaf(in_home));
    ...
    umask(umask_value);
    error(0, 0,
          _(ignoring_input ? N_("ignoring input and appending output to %s")
  ...
  if (redirecting_stderr) {
    ...
    if (0 <= saved_stderr_fd && set_cloexec_flag(saved_stderr_fd, true) != 0)
      error(exit_internal_failure, errno,
            _("failed to set the copy of stderr to close on exec"));

    if (!redirecting_stdout)
      error(0, 0, _(ignoring_input
