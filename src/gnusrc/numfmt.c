...
static char const *const scale_from_args[] = {"none", "auto",  "si",
                                              "iec",  "iec-i", NULL};

static enum scale_type const scale_from_types[] = {
    scale_none, scale_auto, scale_SI, scale_IEC, scale_IEC_I};

static char const *const scale_to_args[] = {"none", "si", "iec", "iec-i", NULL};
...
static char const *const round_args[] = {"up",           "down",    "from-zero",
                                         "towards-zero", "nearest", NULL};
...
static inline const char *suffix_power_char(unsigned int power) {
  switch (power) {
  case 0:
    return "";
...
static long double expld(long double val, unsigned int base,
...
static inline long double _GL_ATTRIBUTE_CONST simple_round(long double val,
                                                           enum round_type t) {
  intmax_t rval;
...
static enum simple_strtod_error simple_strtod_int(const char *input_str,
                                                  char **endptr,
                                                  long double *value,
                                                  bool *negative) {
...
/* Read a floating-point INPUT_STR represented as "NNNN[.NNNNN]",
   ...
   Returns:
      ...
      SSE_OVERFLOW          - if more than 27 digits (999Y) were used.
      SSE_INVALID_NUMBER    - if no digits were found.  */
static enum simple_strtod_error simple_strtod_float(const char *input_str,
                                                    char **endptr,
                                                    long double *value,
                                                    size_t *precision) {
  bool negative;
...
simple_strtod_human(const char *input_str, char **endptr, long double *value,
...
static void double_to_human(long double val, int precision, char *buf,
  ...
  char fmt[64];
  verify(sizeof(fmt) >
         (INT_BUFSIZE_BOUND(zero_padding_width) + INT_BUFSIZE_BOUND(precision) +
          10 /* for %.Lf  etc.  */));
  ...
  *pfmt++ = '%';
  ...
  if (scale == scale_none) {
    ...
    devmsg((group) ? "  no scaling, returning (grouped) value: %'.*Lf\n"
...
static uintmax_t unit_to_umax(const char *n_string) {
  strtol_error s_err;
...
static void parse_format_string(char const *fmt) {
  ...
  for (i = 0; !(fmt[i] == '%' && fmt[i + 1] != '%'); i += (fmt[i] == '%') + 1) {
    if (!fmt[i])
      error(EXIT_FAILURE, 0, _("format %s has no %% directive"), quote(fmt));
    prefix_len++;
  }
  ...
  if (errno == ERANGE)
    error(EXIT_FAILURE, 0, _("invalid format %s (width overflow)"), quote(fmt));
  ...
  if (fmt[i] == '.') {
    ...
    if (errno == ERANGE || user_precision < 0 || SIZE_MAX < user_precision ||
      ...
      error(EXIT_FAILURE, 0, _("invalid precision in format %s"), quote(fmt));
    }
    i = endptr - fmt;
  }
...
static int process_suffixed_number(char *text, long double *result,
  ...
  if (e == SSE_OK_PRECISION_LOSS && debug)
    error(0, 0, _("large input value %s: possible precision loss"), quote(p));
...
static bool _GL_ATTRIBUTE_PURE include_field(size_t field) {
...
static bool process_field(char *text, size_t field) {
  long double val = 0;
  size_t precision = 0;
...
static int process_line(char *line, bool newline) {
  char *next;
  size_t field = 0;
  bool valid_number = true;
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    case FROM_OPTION:
      scale_from =
          XARGMATCH("--from", optarg, scale_from_args, scale_from_types);
      break;
    ...
    case PADDING_OPTION:
      if (xstrtol(optarg, NULL, 10, &padding_width, "") != LONGINT_OK ||
          padding_width == 0)
        error(EXIT_FAILURE, 0, _("invalid padding value %s"), quote(optarg));
      if (padding_width < 0) {
        padding_alignment = MBS_ALIGN_LEFT;
    ...
    case 'd':
      ...
      if (optarg[0] != '\0' && optarg[1] != '\0')
        error(EXIT_FAILURE, 0, _("the delimiter must be a single character"));
      delimiter = optarg[0];
      break;
    ...
    case HEADER_OPTION:
      if (optarg) {
        if (xstrtoumax(optarg, NULL, 10, &header, "") != LONGINT_OK ||
            header == 0)
          error(EXIT_FAILURE, 0, _("invalid header value %s"), quote(optarg));
      } else {
        header = 1;
      }
      break;
