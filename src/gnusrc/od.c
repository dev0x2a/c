...
/* The number of octal digits required to represent the largest
   address value.  */
#define MAX_ADDRESS_LENGTH ((sizeof(uintmax_t) * CHAR_BIT + CHAR_BIT - 1) / 3)
...
static char const *const endian_args[] = {"little", "big", NULL};
...
#define PRINT_FIELDS(N, T, FMT_STRING, ACTION)                                 \
  static void N(size_t fields, size_t blank, void const *block,                \
...
#define PRINT_FLOATTYPE(N, T, FTOASTR, BUFSIZE)                                \
  PRINT_FIELDS(N, T, fmt_string _GL_UNUSED, char buf[BUFSIZE];                 \
...
static bool simple_strtoul(const char *s, const char **p,
...
static bool decode_one_format(const char *s_orig, const char *s,
                              const char **next, struct tspec *tspec) {
...
#define ISPEC_TO_FORMAT(Spec, Min_format, Long_format, Max_format)             \
  ...
  case 'f':
    ...
    default:
      ...
      if (p == s)
        ...
        if (size > MAX_FP_TYPE_SIZE || fp_type_size[size] == NO_SIZE) {
          error(0, 0, _("invalid type string %s;\n"
  ...
  default:
    error(0, 0, _("invalid character '%c' in type string %s"), *s,
          quote(s_orig));
    return false;
  }
...
static bool decode_format_string(const char *s) {
  const char *s_orig = s;
  assert(s != NULL);
...
static void format_address_none(uintmax_t address _GL_UNUSED,
                                char c _GL_UNUSED) {}
...
static bool read_block(size_t n, char *block, size_t *n_bytes_in_buffer) {
  bool ok = true;
...
static int _GL_ATTRIBUTE_PURE get_lcm(void) {
  size_t i;
  int l_c_m = 1;
...
static bool parse_old_offset(const char *s, uintmax_t *offset) {
  int radix;
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    case 'A':
      ...
      case 'n':
        ...
        break;
      default:
        error(EXIT_FAILURE, 0, _("invalid output address radix '%c';\
 it must be one character from [doxn]"),
    ...
    case 'N':
      ...
      s_err = xstrtoumax(optarg, NULL, 0, &max_bytes_to_format, multipliers);
      if (s_err != LONGINT_OK)
        xstrtol_fatal(s_err, oi, c, long_options, optarg);
      break;
...
#undef CASE_OLD_ARG
  ...
  if (flag_dump_strings && n_specs > 0)
    error(EXIT_FAILURE, 0, _("no type may be specified when dumping strings"));
  ...
  */
  ...
  if (!modern || traditional) {
    ...
    if (traditional && 1 < n_files) {
      ...
      error(0, 0, "%s", _("compatibility mode supports at most one file"));
      usage(EXIT_FAILURE);
    }
  }
  ...
  for (i = 0; i < n_specs; i++) {
    ...
    printf("%d: fmt=\"%s\" in_width=%d out_width=%d pad=%d\n", i,
           spec[i].fmt_string, width_bytes[spec[i].size], spec[i].field_width,
           spec[i].pad_width);
  }
#endif
