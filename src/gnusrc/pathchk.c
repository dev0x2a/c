...
#define AUTHORS                                                                \
  proper_name("Paul Eggert"), proper_name("David MacKenzie"),                  \
...
enum { PORTABILITY_OPTION = CHAR_MAX + 1 };
...
static bool no_leading_hyphen(char const *file) {
  char const *p;
...
static bool portable_chars_only(char const *file, size_t filelen) {
  ...
  if (*invalid) {
    ...
    error(0, 0, _("nonportable character %s in file name %s"),
...
static char *_GL_ATTRIBUTE_PURE component_start(char *f) {
  while (*f == '/')
    f++;
  return f;
}
...
static size_t _GL_ATTRIBUTE_PURE component_len(char const *f) {
  size_t len;
...
static bool validate_file_name(char *file, bool check_basic_portability,
  ...
  if (check_basic_portability ||
    ...
    if (check_basic_portability)
      ...
      if (size < 0 && errno != 0) {
        error(0, errno, _("%s: unable to determine maximum file name length"),
              dir);
        return false;
      }
    ...
    if (maxsize <= filelen) {
      ...
      error(0, 0, _("limit %lu exceeded by length %lu of file name %s"), maxlen,
            len, quoteaf(file));
      return false;
    }
  }
  ...
  if (check_component_lengths) {
    ...
    for (start = file; *(start = component_start(start));) {
      ...
      if (name_max < length) {
        ...
        char c = start[len];
        start[len] = '\0';
        error(0, 0, _("limit %lu exceeded by length %lu "
