...
static struct option const longopts[] = {{GETOPT_HELP_OPTION_DECL},
                                         {GETOPT_VERSION_OPTION_DECL},
                                         {NULL, 0, NULL, 0}};

/* Count and return the number of ampersands in STR.  */

static size_t _GL_ATTRIBUTE_PURE count_ampersands(const char *str) {
  size_t count = 0;
  do {
    if (*str == '&')
      count++;
...
static char *create_fullname(const char *gecos_name, const char *user_name) {
...
static const char *idle_string(time_t when) {
  static time_t now = 0;
...
static void short_pinky(const char *filename, const int argc_names,
