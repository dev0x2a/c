...
static char const *const cfcc_msg = N_(
...
static void verify_numeric(const char *s, const char *end) {
  if (errno) {
...
#define STRTOX(TYPE, FUNC_NAME, LIB_FUNC_EXPR)                                 \
  static TYPE FUNC_NAME(char const *s) {                                       \
    char *end;                                                                 \
    TYPE val;                                                                  \
                                                                               \
    if ((*s == '\"' || *s == '\'') && *(s + 1)) {                              \
...
static int print_esc(const char *escstart, bool octal_0) {
  ...
  if (*p == 'x') {
    ...
    for (esc_length = 0, ++p; esc_length < 2 && isxdigit(to_uchar(*p));
         ++esc_length, ++p)
...
static void print_direc(const char *start, size_t length, char conversion,
...
static int print_formatted(const char *format, int argc, char **argv) {
  ...
  for (f = format; *f; ++f) {
    ...
    case '%':
      ...
      if (*f == 'q') {
        if (argc > 0) {
          fputs(quotearg_style(shell_escape_quoting_style, *argv), stdout);
          ++argv;
          --argc;
        }
        break;
    ...
    no_more_flag_characters:
      ...
      if (*f == '*') {
        ...
        if (argc > 0) {
          ...
          if (INT_MIN <= width && width <= INT_MAX)
            ...
            error(EXIT_FAILURE, 0, _("invalid field width: %s"), quote(*argv));
          ++argv;
          --argc;
        } else
          field_width = 0;
        ...
        if (*f == '*') {
          ...
          if (argc > 0) {
            ...
            if (prec < 0) {
              ...
              error(EXIT_FAILURE, 0, _("invalid precision: %s"), quote(*argv));
            else
              precision = prec;
            ++argv;
            --argc;
...

      {
        ...
        if (!ok[conversion])
          error(EXIT_FAILURE, 0, _("%.*s: invalid conversion specification"),
...
int main(int argc, char **argv) {
  ...
  if (argc > 0)
    error(0, 0, _("warning: ignoring excess arguments, starting with %s"),
          quote(argv[0]));
