...
static int
    truncation_string_length; /* length of string that flags truncation */
...
static void ATTRIBUTE_NORETURN matcher_error(void) {
...
static char *copy_unescaped_string(const char *string) {
  ...
  while (*string) {
    if (*string == '\\') {
      ...
      case 'x': /* \xhhh escape, 3 chars maximum */
        value = 0;
        for (length = 0, string++; length < 3 && isxdigit(to_uchar(*string));
             length++, string++)
      ...
      case '0': /* \0ooo escape, 3 chars maximum */
        value = 0;
        for (length = 0, string++; length < 3 && ISODIGIT(*string);
             length++, string++)
...
#define second ((const WORD *)void_second)
  ...
  if (ignore_case) {
    ...
    for (counter = 0; counter < length; counter++) {
      value =
          (to_uchar(first->start[counter]) - to_uchar(second->start[counter]));
      if (value != 0)
        return value;
    }
  }
...
static int _GL_ATTRIBUTE_PURE search_table(WORD *word, WORD_TABLE *table) {
...
static void digest_break_file(const char *file_name) {
...
static void digest_word_file(const char *file_name, WORD_TABLE *table) {
  BLOCK file_contents; /* to receive a copy of the file */
  char *cursor;        /* cursor in file copy */
  ...
  while (cursor < file_contents.end) {
    ...
    if (cursor > word_start) {
      if (table->length == table->alloc) {
        ...
        table->start =
            xrealloc(table->start, table->alloc * sizeof *table->start);
      }
...
static void find_occurs_in_text(size_t file_index) {
  ...
  for (cursor = text_buffer->start; cursor < text_buffer->end;
    ...
    while (1) {
      ...
      if (number_of_occurs[0] == occurs_alloc[0]) {
        if ((SIZE_MAX / sizeof *occurs_table[0] - 1) / 2 < occurs_alloc[0])
          xalloc_die();
      ...
      if (auto_reference) {
        ...
        occurs_cursor->reference = (DELTA)(line_start - possible_key.start);
...
static char const *const format_args[] = {"roff", "tex", NULL};
  ...
  while (optchar = getopt_long(argc, argv, "AF:GM:ORS:TW:b:i:fg:o:trw:",
    ...
    case 'g': {
      ...
      if (xstrtoul(optarg, NULL, 0, &tmp_ulong, NULL) != LONGINT_OK ||
        ...
        error(EXIT_FAILURE, 0, _("invalid gap width: %s"), quote(optarg));
      gap_size = tmp_ulong;
      break;
    }
    ...
    case 'w': {
      ...
      if (xstrtoul(optarg, NULL, 0, &tmp_ulong, NULL) != LONGINT_OK ||
          !(0 < tmp_ulong && tmp_ulong <= INT_MAX))
        error(EXIT_FAILURE, 0, _("invalid line width: %s"), quote(optarg));
