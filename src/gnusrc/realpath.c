...
enum { RELATIVE_TO_OPTION = CHAR_MAX + 1, RELATIVE_BASE_OPTION };
...
static char *realpath_canon(const char *fname, int can_mode) {
...
static bool _GL_ATTRIBUTE_PURE path_prefix(const char *prefix,
                                           const char *path) {
...
static bool process_path(const char *fname, int can_mode) {
