...
static int _GL_ATTRIBUTE_PURE path_common_prefix(const char *path1,
                                                 const char *path2) {
  int i = 0;
  int ret = 0;
...
static bool buffer_or_output(const char *str, char **pbuf, size_t *plen) {
  if (*pbuf) {
...
bool relpath(const char *can_fname, const char *can_reldir, char *buf,
             size_t len) {
  bool buf_err = false;
