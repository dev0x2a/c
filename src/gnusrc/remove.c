...
static enum RM_status prompt(FTS const *fts, FTSENT const *ent, bool is_dir,
  ...
  if (write_protected || x->interactive == RMI_ALWAYS) {
    ...
    /* Issue the prompt.  */
    if (dirent_type == DT_DIR && mode == PA_DESCEND_INTO_DIR && !is_empty)
      fprintf(stderr,
              (write_protected
...
static inline bool nonexistent_file_errno(int errnum) {
...
static inline bool ignorable_missing(struct rm_options const *x, int errnum) {
...
static enum RM_status excise(FTS *fts, FTSENT *ent, struct rm_options const *x,
  ...
  if (ent->fts_info == FTS_DNR && (errno == ENOTEMPTY || errno == EISDIR ||
...
static enum RM_status rm_fts(FTS *fts, FTSENT *ent,
  ...
  case FTS_D: /* preorder directory */
    ...
    if (ent->fts_level == FTS_ROOTLEVEL) {
      ...
      if (dot_or_dotdot(last_component(ent->fts_accpath))) {
        error(0, 0, _("refusing to remove %s or %s directory: skipping %s"),
  ...
  case FTS_DEFAULT: /* none of the above */
  {
    /* With --one-file-system, do not attempt to remove a mount point.
       ...
       the mount point.  */
    if (ent->fts_info == FTS_DP && x->one_file_system &&
  ...
  case FTS_ERR:
    ...
    error(0, ent->fts_errno, _("traversal failed: %s"), quotef(ent->fts_path));
    fts_skip_tree(fts, ent);
    return RM_ERROR;
  ...
  default:
    error(0, 0, _("unexpected failure: fts_info=%d: %s\n"
                  "please report to %s"),
          ent->fts_info, quotef(ent->fts_path), PACKAGE_BUGREPORT);
    abort();
  }
}
