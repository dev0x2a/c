...
#define AUTHORS                                                                \
  proper_name("Paul Rubin"), proper_name("David MacKenzie"),                   \
      proper_name("Richard M. Stallman"), proper_name("Jim Meyering")
...
static char const *const interactive_args[] = {"never",  "no",  "none", "once",
                                               "always", "yes", NULL};
...
static void diagnose_leading_hyphen(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (arg[0] == '-' && arg[1] && lstat(arg, &st) == 0) {
      fprintf(stderr, _("Try '%s ./%s' to remove the file %s.\n"), argv[0],
