...
enum { IGNORE_FAIL_ON_NON_EMPTY_OPTION = CHAR_MAX + 1 };
...
static bool errno_may_be_empty(int error_number) {
  switch (error_number) {
  case EACCES:
  case EPERM:
  case EROFS:
...
static bool ignorable_failure(int error_number, char const *dir) {
  return (ignore_fail_on_non_empty &&
          (errno_rmdir_non_empty(error_number) ||
           (errno_may_be_empty(error_number) && is_empty_dir(AT_FDCWD, dir))));
}
  ...
  while (1) {
    ...
    if (!ok) {
      ...
      if (ignorable_failure(errno, dir)) {
        ...
        error(0, errno, _("failed to remove directory %s"), quoteaf(dir));
      }
      break;
    }
  }
