...
int main(int argc, char **argv) {
  ...
  if (context) {
    ...
    if (user && context_user_set(con, user))
      error(EXIT_FAILURE, errno, _("failed to set new user: %s"), quote(user));
    if (type && context_type_set(con, type))
      error(EXIT_FAILURE, errno, _("failed to set new type: %s"), quote(type));
    if (range && context_range_set(con, range))
      error(EXIT_FAILURE, errno, _("failed to set new range: %s"),
            quote(range));
    if (role && context_role_set(con, role))
      error(EXIT_FAILURE, errno, _("failed to set new role: %s"), quote(role));
  }
