...
static operand scan_arg(const char *arg) {
  operand ret;
...
static void ATTRIBUTE_NORETURN io_error(void) {
...
static char const *_GL_ATTRIBUTE_PURE trim_leading_zeros(char const *s) {
  char const *p = s;
  while (*s == '0')
    ++s;
...
static bool seq_fast(char const *a, char const *b) {
...
static bool _GL_ATTRIBUTE_PURE all_digits_p(char const *s) {
  size_t n = strlen(s);
...
int main(int argc, char **argv) {
  ...
  if (all_digits_p(argv[optind]) &&
      ...
      (n_args < 3 ||
       (STREQ("1", argv[optind + 1]) && all_digits_p(argv[optind + 2]))) &&
  ...
  if ((isfinite(first.value) && first.precision == 0) && step.precision == 0 &&
      last.precision == 0 && 0 <= first.value && step.value == 1 &&
      0 <= last.value && !equal_width && !format_str &&
      strlen(separator) == 1) {
    char *s1;
    char *s2;
