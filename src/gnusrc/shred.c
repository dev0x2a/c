...
static char const *const remove_args[] = {"unlink", "wipe", "wipesync", NULL};
...
enum { RANDOM_SOURCE_OPTION = CHAR_MAX + 1 };
 ...
 */
static bool periodic_pattern(int type) {
  if (type <= 0)
    return false;
...
static void passname(unsigned char const *data, char name[PASS_NAME_SIZE]) {
  if (data)
...
static bool ignorable_sync_errno(int errno_val) {
  return (errno_val == EINVAL || errno_val == EBADF
          /* HP-UX does this */
          || errno_val == EISDIR);
}
...
static void direct_mode(int fd, bool enable) {
  if (O_DIRECT) {
    int fd_flags = fcntl(fd, F_GETFL);
    if (0 < fd_flags) {
      int new_flags = (enable ? (fd_flags | O_DIRECT) : (fd_flags & ~O_DIRECT));
...
static bool dorewind(int fd, struct stat const *st) {
...
#define NONPERIODIC_OUTPUT_SIZE (64 * 1024)
  ...
  size_t output_size =
      periodic_pattern(type) ? PERIODIC_OUTPUT_SIZE : NONPERIODIC_OUTPUT_SIZE;
...
#define PATTERNBUF_SIZE (PAGE_ALIGN_SLOP + FILLPATTERN_SIZE)
  ...
  while (true) {
    ...
    for (soff = 0; soff < lim; soff += ssize) {
      ...
      if (0 < ssize)
        ...
        if (!known(size) && (ssize == 0 || errno == ENOSPC)) {
          ...
          error(0, errnum, _("%s: error writing at offset %s"), qname,
    ...
    if (n &&
        ((done && *previous_human_offset) || thresh <= (now = time(NULL)))) {
      ...
      if (done || !STREQ(previous_human_offset, human_offset)) {
        if (!known(size))
          ...
          int percent = (size == 0 ? 100 : (off <= TYPE_MAXIMUM(uintmax_t) / 100
                                                ? off * 100 / size
                                                : off / (size / 100)));
          ...
          error(0, 0, _("%s: pass %lu/%lu (%s)...%s/%s %d%%"), qname, k, n,
                pass_string, human_offset, human_size, percent);
        }
...
static int const patterns[] = {
    ...
    8, 0x1000, 0x1249, 0x1492, 0x16DB, 0x1924, 0x1B6D, 0x1DB6, 0x1FFF, 14,
    0x1111, 0x1222, 0x1333, 0x1444, 0x1555, 0x1666, 0x1777, 0x1888, 0x1999,
    0x1AAA, 0x1BBB, 0x1CCC, 0x1DDD, 0x1EEE, -1, /* 1 random pass */
    0                                           /* End */
};
...
static void genpattern(int *dest, size_t num, struct randint_source *s) {
  ...
  while (true) {
    ...
    if (!k) { /* Loop back to the beginning */
      ...
      randpasses += n;
      break;
    } else { /* Pad out with n of the k available */
      do {
 ...
 */
static bool do_wipefd(int fd, char const *qname, struct randint_source *s,
  ...
  if ((S_ISCHR(st.st_mode) && isatty(fd)) || S_ISFIFO(st.st_mode) ||
      S_ISSOCK(st.st_mode)) {
  ...
  } else if (S_ISREG(st.st_mode) && st.st_size < MIN(ST_BLKSIZE(st), size))
    i_size = st.st_size;
  ...
  if (flags->remove_file && ftruncate(fd, 0) != 0 && S_ISREG(st.st_mode)) {
...
static bool wipefd(int fd, char const *qname, struct randint_source *s,
                   struct Options const *flags) {
  int fd_flags = fcntl(fd, F_GETFL);
...
static bool incname(char *name, size_t len) {
  while (len--) {
 ...
 */
static bool wipename(char *oldname, char const *qoldname,
                     struct Options const *flags) {
 ...
 */
static bool wipefile(char *name, char const *qname, struct randint_source *s,
                     struct Options const *flags) {
  bool ok;
  int fd;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "fn:s:uvxz", long_opts, NULL)) != -1) {
    ...
    case 'n':
      flags.n_iterations =
          xdectoumax(optarg, 0, MIN(ULONG_MAX, SIZE_MAX / sizeof(int)), "",
    ...
    case 'u':
      if (optarg == NULL)
        ...
        flags.remove_file =
            XARGMATCH("--remove", optarg, remove_args, remove_methods);
      break;
