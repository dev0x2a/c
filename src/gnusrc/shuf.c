...
static char *next_line(char *line, char eolbyte, size_t n) {
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "ei:n:o:rz", long_opts, NULL)) != -1)
    ...
    case 'i': {
      ...
      if (p) {
        *p = '\0';
        lo_input =
            xdectoumax(optarg, 0, SIZE_MAX, "", _("invalid input range"), 0);
        *p = '-';
        hi_optarg = p + 1;
      }

      hi_input =
          xdectoumax(hi_optarg, 0, SIZE_MAX, "", _("invalid input range"), 0);
    ...
    case 'n': {
      ...
      if (e == LONGINT_OK)
        ...
        error(EXIT_FAILURE, 0, _("invalid line count: %s"), quote(optarg));
    } break;
  ...
  /* Close stdin now, rather than earlier, so that randint_all_new
     ...
     stdin.  */
  if (!(echo || input_range) && (fclose(stdin) != 0))
    error(EXIT_FAILURE, errno, _("read error"));
  ...
  if (repeat) {
    if (head_lines == 0)
      ...
      if (input_range)
        i = write_random_numbers(randint_source, head_lines, lo_input, hi_input,
                                 eolbyte);
      else
