...
static inline bool ignorable(int err) { return filter_command && err == EPIPE; }
  ...
  if (pid > 0) {
    ...
    if (WIFSIGNALED(wstatus)) {
      ...
      if (sig != SIGPIPE) {
        ...
        error(sig + 128, 0, _("with FILE=%s, signal %s from command: %s"),
              quotef(name), signame, filter_command);
      }
    } else if (WIFEXITED(wstatus)) {
      int ex = WEXITSTATUS(wstatus);
      if (ex != 0)
        error(ex, 0, _("with FILE=%s, exit %d from command: %s"), quotef(name),
              ex, filter_command);
    } else {
      /* shouldn't happen.  */
      error(EXIT_FAILURE, 0, _("unknown status from command (0x%X)"),
            wstatus + 0u);
    }
  }
}
...
static bool cwrite(bool new_file_flag, const char *bp, size_t bytes) {
  if (new_file_flag) {
      ...
      if (k) {
        ...
        if (unbuffered) {
          ...
          if (full_write(files[i_file].ofd, bp, to_write) != to_write &&
              !ignorable(errno)) {
            error(EXIT_FAILURE, errno, "%s", quotef(files[i_file].of_name));
          }
...
static void parse_chunk(uintmax_t *k_units, uintmax_t *n_units, char *slash) {
  *n_units = xdectoumax(slash + 1, 1, UINTMAX_MAX, "",
  ...
  if (slash != optarg) /* a leading number is specified.  */
  {
    *slash = '\0';
    *k_units =
        xdectoumax(optarg, 1, *n_units, "", _("invalid chunk number"), 0);
  }
}
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    c = getopt_long(argc, argv, "0123456789C:a:b:del:n:t:u", longopts, NULL);
    if (c == -1)
      break;
    ...
    case ADDITIONAL_SUFFIX_OPTION:
      if (last_component(optarg) != optarg) {
        error(0, 0, _("invalid suffix %s, contains directory separator"),
              quote(optarg));
        usage(EXIT_FAILURE);
      }
    ...
    case 'C':
      ...
      n_units = xdectoumax(optarg, 1, MIN(SIZE_MAX, OFF_T_MAX), multipliers,
                           _("invalid number of bytes"), 0);
      break;
    ...
    case 't': {
      ...
      if (0 <= eolchar && neweol != eolchar) {
        error(EXIT_FAILURE, 0, _("multiple separator characters specified"));
      }
    ...
    case '9':
      ...
      if (!DECIMAL_DIGIT_ACCUMULATE(n_units, c - '0', uintmax_t)) {
        ...
        error(EXIT_FAILURE, 0, _("line count option -%s%c... is too large"),
    ...
    case 'd':
      ...
      if (optarg) {
        if (strlen(optarg) != strspn(optarg, suffix_alphabet)) {
          error(0, 0, _("%s: invalid start value for numerical suffix"),
                quote(optarg));
          usage(EXIT_FAILURE);
        } else {
    ...
    case IO_BLKSIZE_OPTION:
      in_blk_size = xdectoumax(optarg, 1, SIZE_MAX - page_size, multipliers,
  ...
  /* Open the input file.  */
  if (!STREQ(infile, "-") && fd_reopen(STDIN_FILENO, infile, O_RDONLY, 0) < 0)
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines) {
    file_size = input_file_size(STDIN_FILENO, &in_stat_buf, buf, in_blk_size);
    if (file_size < 0)
    ...
    if (OFF_T_MAX < n_units) {
      char buffer[INT_BUFSIZE_BOUND(uintmax_t)];
      error(EXIT_FAILURE, EOVERFLOW, "%s: %s", _("invalid number of chunks"),
