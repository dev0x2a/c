...
static int ATTRIBUTE_WARN_UNUSED_RESULT statfs(char const *filename,
                                               struct fs_info *buf) {
  dev_t device = dev_for_path(filename);
  if (device < 0) {
    errno = (device == B_ENTRY_NOT_FOUND
                 ? ENOENT
                 : device == B_BAD_VALUE
                       ? EINVAL
                       : device == B_NAME_TOO_LONG
                             ? ENAMETOOLONG
                             : device == B_NO_MEMORY
                                   ? ENOMEM
                                   : device == B_FILE_ERROR ? EIO : 0);
    return -1;
  }
...
human_fstype(STRUCT_STATVFS const *statfsbuf) {
  ...
  */
  ...
  default: {
    ...
    static char
        buf[sizeof "UNKNOWN (0x%lx)" - 3 + (sizeof type * CHAR_BIT + 3) / 4];
...
static char *ATTRIBUTE_WARN_UNUSED_RESULT human_time(struct timespec t) {
  ...
  static char str[intmax_bufsize + INT_STRLEN_BOUND(int) /* YYYY */
...
static bool ATTRIBUTE_WARN_UNUSED_RESULT print_statfs(char *pformat,
                                                      size_t prefix_len,
                                                      unsigned int m, int fd,
                                                      char const *filename,
                                                      void const *data) {
...
static struct timespec get_birthtime(int fd, char const *filename,
...
static bool print_stat(char *pformat, size_t prefix_len, unsigned int m, int fd,
  ...
  case 'N':
    ...
    if (S_ISLNK(statbuf->st_mode)) {
      ...
      if (linkname == NULL) {
        error(0, errno, _("cannot read symbolic link %s"), quoteaf(filename));
        return true;
      }
      printf(" -> ");
  ...
  for (b = format; *b; b++) {
    ...
    case '%': {
      ...
      case '%':
        if (0 < len) {
          dest[len + 1] = *fmt_char;
          dest[len + 2] = '\0';
          error(EXIT_FAILURE, 0, _("%s: invalid directive"), quote(dest));
        }
        putchar('%');
        break;
      default:
        fail |= print_func(dest, len + 1, fmt_code, fd, filename, data);
        break;
      }
      break;
    }
    ...
    case '\\':
      ...
      if (isodigit(*b)) {
        ...
        for (++b; esc_length < 3 && isodigit(*b); ++esc_length, ++b) {
...
static bool ATTRIBUTE_WARN_UNUSED_RESULT do_statfs(char const *filename,
                                                   char const *format) {
...
static bool ATTRIBUTE_WARN_UNUSED_RESULT do_stat(char const *filename,
                                                 char const *format,
                                                 char const *format2) {
  ...
  else if ((follow_links ? stat(filename, &statbuf)
