...
static void set_program_path(const char *arg) {
...
static void set_LD_PRELOAD(void) {
  ...
  char const *const search_path[] = {program_path, PKGLIBEXECDIR, NULL};
  ...
  if (ret != 0) {
    error(EXIT_CANCELED, errno, _("failed to update the environment with %s"),
          quote(LD_PRELOAD));
  }
}
...
static bool set_libstdbuf_options(void) {
  bool env_set = false;
  size_t i;
  ...
  for (i = 0; i < ARRAY_CARDINALITY(stdbuf); i++) {
    if (stdbuf[i].optarg) {
      ...
      if (*stdbuf[i].optarg == 'L')
        ret = asprintf(&var, "%s%c=L", "_STDBUF_", toupper(stdbuf[i].optc));
      else
        ret = asprintf(&var, "%s%c=%" PRIuMAX, "_STDBUF_",
                       toupper(stdbuf[i].optc), (uintmax_t)stdbuf[i].size);
      if (ret < 0)
        xalloc_die();

      if (putenv(var) != 0) {
        error(EXIT_CANCELED, errno,
              _("failed to update the environment with %s"), quote(var));
      }
