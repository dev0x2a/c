...
static void set_control_char(struct control_info const *info, const char *arg,
                             struct termios *mode);
...
static struct option const longopts[] = {
    {"all", no_argument, NULL, 'a'},        {"save", no_argument, NULL, 'g'},
  ...
  for (k = 1; k < argc; k++) {
    ...
    if (!match_found || not_set_attr) {
      ...
      /* This is the BSD interface to "extproc".
        ...
        ++k;
        set_window_size(integer_arg(argv[k], INT_MAX), -1, device_name);
        ...
        ++k;
        set_window_size(-1, integer_arg(argv[k], INT_MAX), device_name);
...
static void display_window_size(bool fancy, char const *device_name) {
  ...
  if (get_win_size(STDIN_FILENO, &win)) {
    if (errno != EINVAL)
      error(EXIT_FAILURE, errno, "%s", quotef(device_name));
    if (!fancy)
      error(EXIT_FAILURE, 0, _("%s: no size information for this device"),
            quotef(device_name));
  } else {
...
static int screen_columns(void) {
...
#endif
  {
    ...
    if (!(col_string != NULL &&
          ...
          0 < n_columns && n_columns <= INT_MAX))
      n_columns = 80;
    return n_columns;
  }
}

static tcflag_t *_GL_ATTRIBUTE_PURE mode_type_flag(enum mode_type type,
                                                   struct termios *mode) {
  switch (type) {
  case control:
    return &mode->c_cflag;
...
#if VEOF == VMIN
    if ((mode->c_lflag & ICANON) == 0 && (STREQ(control_info[i].name, "eof") ||
...
/* If eof uses the same slot as min, only print whichever applies.  */
  ...
  if ((mode->c_lflag & ICANON) == 0)
#endif
    wrapf("min = %lu; time = %lu;", (unsigned long int)mode->c_cc[VMIN],
          (unsigned long int)mode->c_cc[VTIME]);
  if (current_col != 0)
    putchar('\n');
  current_col = 0;
...
static struct speed_map const speeds[] = {{"0", B0, 0},
                                          {"50", B50, 50},
                                          {"75", B75, 75},
                                          {"110", B110, 110},
                                          {"134", B134, 134},
...
static speed_t _GL_ATTRIBUTE_PURE string_to_baud(const char *arg) {
  int i;
...
static unsigned long int _GL_ATTRIBUTE_PURE baud_to_value(speed_t speed) {
  int i;
...
static unsigned long int integer_arg(const char *s, unsigned long int maxval) {
