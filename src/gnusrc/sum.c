...
static struct option const longopts[] = {{"sysv", no_argument, NULL, 's'},
...
static bool bsd_sum_file(const char *file, int print_name)
{
  FILE *fp;
  ...
  printf("%05d %5s",
      checksum,human_readable(total_bytes,hbuf,human_ceiling,1,1024));
  if (print_name > 1)
    printf(" %s", file);
  putchar('\n');
  return true;
...
/* Calculate and print the checksum and the size in 512-byte blocks
   ...
   Return true if successful */
static bool sysv_sum_file(const char *file, int print_name)
{
  int fd, r, checksum;
  ...
  printf("%d %s",
      checksum,human_readable(total_bytes,hbuf,human_ceiling,1,512));
  if (print_name)
    printf(" %s", file);
  putchar('\n');
  return true;
