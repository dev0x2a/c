...
#define AUTHORS proper_name("Jim Meyering"), proper_name("Giuseppe Scrivano")
...
static bool sync_arg(enum sync_mode mode, char const *file) {
  bool ret = true;
  ...
  if (fdflags == -1 || fcntl(fd, F_SETFL, fdflags & ~O_NONBLOCK) < 0) {
    error(0, errno, _("couldn't reset non-blocking mode %s"), quoteaf(file));
    ret = false;
  }
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "df", long_options, NULL)) != -1) {
    switch (c) {
    case 'd':
      arg_data = true;
      break;
