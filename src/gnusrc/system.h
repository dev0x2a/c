...
#define X2REALLOC(P, PN) ((void)verify_true(sizeof *(P) == 1), x2realloc(P, PN))
...
static inline bool is_empty_dir(int fd_cwd, char const *dir) {
  ...
  int saved_errno;
  int fd = openat(fd_cwd, dir, (O_RDONLY | O_DIRECTORY | O_NOCTTY | O_NOFOLLOW |
                                O_NONBLOCK));
...
static inline size_t _GL_ATTRIBUTE_CONST gcd(size_t u, size_t v) {
  do {
    size_t t = u % v;
    u = v;
    v = t;
...
static inline size_t _GL_ATTRIBUTE_CONST lcm(size_t u, size_t v) {
  return u * (v / gcd(u, v));
}
...
static inline bool _GL_ATTRIBUTE_PURE is_nul(void const *buf, size_t length) {
...
static inline bool usable_st_size(struct stat const *sb) {
  return (S_ISREG(sb->st_mode) || S_ISLNK(sb->st_mode) || S_TYPEISSHM(sb) ||
          S_TYPEISTMO(sb));
}
...
static inline char *stzncpy(char *restrict dest, char const *restrict src,
                            size_t len) {
