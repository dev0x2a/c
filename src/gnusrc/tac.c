...
#define AUTHORS proper_name("Jay Lepreau"), proper_name("David MacKenzie")
...
static bool tac_seekable(int input_fd, const char *file, off_t file_pos) {
  ...
  while (true) {
    ...
    if (sentinel_length == 0) {
      ...
      if (range == 1 || ((ret = re_search(&compiled_separator, G_buffer, i,
        ...
        error(EXIT_FAILURE, 0, _("error in regular expression search"));
      } else {
      ...
      while (*--match_start != first_char ||
             (match_length1 &&
              !STREQ_LEN(match_start + 1, separator1, match_length1)))
        /* Do nothing. */;
    }
...
static void record_or_unlink_tempfile(char const *fn, FILE *fp) {
  if (!file_to_remove) {
    file_to_remove = fn;
    fp_to_close = fp;
...
static void record_or_unlink_tempfile(char const *fn, FILE *fp _GL_UNUSED) {
  unlink(fn);
}
...
static bool temp_stream(FILE **fp, char **file_name) {
  ...
  if (tempfile == NULL) {
    ...
    if (!tmp_fp) {
      error(0, errno, _("failed to open %s for writing"), quoteaf(tempfile));
      close(fd);
      unlink(tempfile);
    Reset:
      free(tempfile);
    ...
    if (fseeko(tmp_fp, 0, SEEK_SET) < 0 || ftruncate(fileno(tmp_fp), 0) < 0) {
      error(0, errno, _("failed to rewind stream for %s"), quoteaf(tempfile));
      return false;
    }
  }
...
static bool tac_file(const char *filename) {
  bool ok;
  off_t file_size;
  int fd;
  ...
  if (is_stdin) {
    ...
    if (fd < 0) {
      error(0, errno, _("failed to open %s for reading"), quoteaf(filename));
      return false;
    }
  }
