...
#define AUTHORS                                                                \
  proper_name("Paul Rubin"), proper_name("David MacKenzie"),                   \
      proper_name("Ian Lance Taylor"), proper_name("Jim Meyering")
...
static char const *const follow_mode_string[] = {"descriptor", "name", NULL};
...
static bool valid_file_spec(struct File_spec const *f) {
...
static off_t xlseek(int fd, off_t offset, int whence, char const *filename) {
  ...
  switch (whence) {
  case SEEK_SET:
    error(0, errno, _("%s: cannot seek to offset %s"), quotef(filename), s);
    break;
  case SEEK_CUR:
...
static bool file_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
  ...
  do {
    ...
    while (n) {
      ...
      if (n_lines-- == 0) {
        ...
        *read_pos +=
            dump_remainder(pretty_filename, fd, end_pos - (pos + bytes_read));
        return true;
      }
    }
    ...
    if (pos == start_pos) {
      ...
      *read_pos = start_pos + dump_remainder(pretty_filename, fd, end_pos);
      return true;
    }
    pos -= BUFSIZ;
    xlseek(fd, pos, SEEK_SET, pretty_filename);
...
static bool pipe_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  struct linebuffer {
    char buffer[BUFSIZ];
    size_t nbytes;
...
static bool pipe_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  struct charbuffer {
    char buffer[BUFSIZ];
    size_t nbytes;
...
static int start_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  char buffer[BUFSIZ];
...
static int start_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  if (n_lines == 0)
    return 0;
...
static void recheck(struct File_spec *f, bool blocking) {
 ...
 giving up on this name"),
  ...
  if (!ok) {
    ...
    error(0, 0, _("%s has appeared;  following new file"),
          quoteaf(pretty_name(f)));
    ...
    error(0, 0, _("%s has been replaced;  following new file"),
          quoteaf(pretty_name(f)));
...
static bool any_live_files(const struct File_spec *f, size_t n_files) {
  size_t i;
  ...
  while (1) {
    ...
    for (i = 0; i < n_files; i++) {
      ...
      if (f[i].blocking != blocking) {
        ...
        if (old_flags < 0 ||
          ...
          if (S_ISREG(f[i].mode) && errno == EPERM) {
            ...
            error(EXIT_FAILURE, errno, _("%s: cannot change nonblocking mode"),
                  quotef(name));
        } else
      ...
      bytes_read = dump_remainder(
          name, fd, (f[i].blocking ? COPY_A_BUFFER : COPY_TO_EOF));
...
static bool any_non_remote_file(const struct File_spec *f, size_t n_files) {
  size_t i;
...
static bool any_symlinks(const struct File_spec *f, size_t n_files) {
  size_t i;
...
static bool tailable_stdin(const struct File_spec *f, size_t n_files) {
  size_t i;
...
static bool wd_comparator(const void *e1, const void *e2) {
...
static bool tail_forever_inotify(int wd, struct File_spec *f, size_t n_files,
                                 double sleep_interval) {
#if TAIL_TEST_SLEEP
  ...
  for (i = 0; i < n_files; i++) {
    if (!f[i].ignore) {
      ...
      if (follow_mode == Follow_name) {
        ...
        f[i].parent_wd = inotify_add_watch(
            wd, dirlen ? f[i].name : ".",
            (IN_CREATE | IN_DELETE | IN_MOVED_TO | IN_ATTRIB));
    ...
    if (!f[i].ignore) {
      ...
      if (follow_mode == Follow_name)
        ...
        if (stat(f[i].name, &stats) == 0 &&
          ...
          error(0, errno, _("%s was replaced"), quoteaf(pretty_name(&(f[i]))));
          hash_free(wd_to_name);
...
static bool tail_bytes(const char *pretty_filename, int fd, uintmax_t n_bytes,
                       uintmax_t *read_pos) {
  struct stat stats;
  ...
  if (from_start) {
    if (!presume_input_pipe && S_ISREG(stats.st_mode) && n_bytes <= OFF_T_MAX) {
    ...
    off_t end_pos =
        ((!presume_input_pipe && usable_st_size(&stats) && n_bytes <= OFF_T_MAX)
             ? stats.st_size
             : -1);
...
static bool tail_lines(const char *pretty_filename, int fd, uintmax_t n_lines,
                       uintmax_t *read_pos) {
  struct stat stats;
  ...
  if (from_start) {
    ...
    *read_pos += dump_remainder(pretty_filename, fd, COPY_TO_EOF);
    ...
    if (!presume_input_pipe && S_ISREG(stats.st_mode) &&
        (start_pos = lseek(fd, 0, SEEK_CUR)) != -1 &&
        start_pos < (end_pos = lseek(fd, 0, SEEK_END))) {
      *read_pos = end_pos;
      if (end_pos != 0 &&
          !file_lines(pretty_filename, fd, n_lines, start_pos, end_pos,
                      read_pos))
        return false;
    } else {
...
static bool tail(const char *filename, int fd, uintmax_t n_units,
                 uintmax_t *read_pos) {
  *read_pos = 0;
  if (count_lines)
...
static bool tail_file(struct File_spec *f, uintmax_t n_units) {
  int fd;
  bool ok;
  ...
  if (fd == -1) {
    ...
    error(0, errno, _("cannot open %s for reading"), quoteaf(pretty_name(f)));
    ok = false;
  } else {
    uintmax_t read_pos;
    ...
    if (forever) {
      ...
      if (fstat(fd, &stats) < 0) {
        ok = false;
        f->errnum = errno;
        error(0, errno, _("error reading %s"), quoteaf(pretty_name(f)));
...
static bool parse_obsolete_option(int argc, char *const *argv,
                                  uintmax_t *n_units) {
  const char *p;
  const char *n_string;
  ...
  if (n_string == n_string_end)
    *n_units = default_count;
    ...
    error(EXIT_FAILURE, errno, "%s: %s", _("invalid number"), quote(argv[1]));
  }
  ...
  while ((c = getopt_long(argc, argv, "c:n:fFqs:vz0123456789", long_options,
    ...
    case LONG_FOLLOW_OPTION:
      ...
      if (optarg == NULL)
        ...
        follow_mode =
            XARGMATCH("--follow", optarg, follow_mode_string, follow_mode_map);
      break;
    ...
    case 's': {
      ...
      if (!(xstrtod(optarg, NULL, &s, c_strtod) && 0 <= s))
        error(EXIT_FAILURE, 0, _("invalid number of seconds: %s"),
              quote(optarg));
      *sleep_interval = s;
    } break;
...
static size_t ignore_fifo_and_pipe(struct File_spec *f, size_t n_files) {
  ...
  for (i = 0; i < n_files; i++) {
    bool is_a_fifo_or_pipe =
        (STREQ(f[i].name, "-") && !f[i].ignore && 0 <= f[i].fd &&
         (S_ISFIFO(f[i].mode) || (HAVE_FIFO_PIPES != 1 && isapipe(f[i].fd))));
