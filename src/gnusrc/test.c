...
static bool get_mtime(char const *filename, struct timespec *mtime) {
  struct stat finfo;
...
static bool binop(char const *s) {
...
static bool term(void) {
  ...
  if (argv[pos][0] == '(' && argv[pos][1] == '\0') {
    ...
    for (nargs = 1; pos + nargs < argc && !STREQ(argv[pos + nargs], ")");
         nargs++)
      if (nargs == 4) {
        nargs = argc - pos;
        break;
    ...
    if (argv[pos] == 0)
      ...
      test_syntax_error(_("%s expected, found %s"), quote_n(0, ")"),
                        quote_n(1, argv[pos]));
    advance(false);
  }
...
static bool binary_operator(bool l_is_l) {
  int op;
...
static bool unary_operator(void) {
  ...
  case 'f': /* File is a file? */
    ...
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISREG(stat_buf.st_mode));

  case 'd': /* File is a directory? */
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISDIR(stat_buf.st_mode));
  ...
  case 'S': /* File is a socket? */
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISSOCK(stat_buf.st_mode));

  case 'c': /* File is character special? */
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISCHR(stat_buf.st_mode));

  case 'b': /* File is block special? */
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISBLK(stat_buf.st_mode));

  case 'p': /* File is a named pipe? */
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && S_ISFIFO(stat_buf.st_mode));
  ...
  case 'h': /* File is a symbolic link? */
    unary_advance();
    return (lstat(argv[pos - 1], &stat_buf) == 0 && S_ISLNK(stat_buf.st_mode));
...
static bool test_unop(char const *op) {
  if (op[0] != '-')
    return false;
...
#define AUTHORS proper_name("Kevin Braunsdorf"), proper_name("Matthew Bradburn")
