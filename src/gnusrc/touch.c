...
#define AUTHORS                                                                \
  proper_name("Paul Rubin"), proper_name("Arnold Robbins"),                    \
      proper_name("Jim Kingdon"), proper_name("David MacKenzie"),              \
...
static int const time_masks[] = {CH_ATIME, CH_ATIME, CH_ATIME, CH_MTIME,
                                 CH_MTIME};
...
int main(int argc, char **argv) {
  ...
  if (!date_set && 2 <= argc - optind && posix2_version() < 200112 &&
    ...
    if (!getenv("POSIXLY_CORRECT")) {
      ...
      if (tm)
        error(0, 0, _("warning: 'touch %s' is obsolete; use "
