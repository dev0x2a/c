...
static inline bool es_match(struct E_string const *es, size_t i, char c) {
...
static inline bool is_equiv_class_member(unsigned char equiv_class,
                                         unsigned char c) {
...
static bool _GL_ATTRIBUTE_PURE is_char_class_member(enum Char_class char_class,
                                                    unsigned char c) {
  int result;
...
static bool unquote(char const *s, struct E_string *es) {
  size_t i, j;
  size_t len = strlen(s);
...
static bool append_range(struct Spec_list *list, unsigned char first,
                         unsigned char last) {
  struct List_element *new;
...
static bool append_char_class(struct Spec_list *list,
...
static bool append_equiv_class(struct Spec_list *list,
...
static bool find_closing_delim(const struct E_string *es, size_t start_idx,
...
static bool build_spec_list(const struct E_string *es,
        ...
        if (find_closing_delim(es, i + 2, p[i + 1], &closing_delim_idx)) {
          ...
          if (opnd_str_len == 0) {
            if (p[i + 1] == ':')
              ...
              error(0, 0, _("missing equivalence class character '[==]'"));
            return false;
          }
          ...
          if (p[i + 1] == ':') {
            ...
            if (!append_char_class(result, opnd_str, opnd_str_len)) {
              if (star_digits_closebracket(es, i + 2))
                ...
                error(0, 0, _("invalid character class %s"), quote(tmp));
                free(tmp);
                return false;
              }
            }
            ...
            if (!append_equiv_class(result, opnd_str, opnd_str_len)) {
              if (star_digits_closebracket(es, i + 2))
                ...
                char *tmp = make_printable_str(opnd_str, opnd_str_len);
                error(0, 0, _("%s: equivalence class operand must be a single "
                              "character"),
                      tmp);
                free(tmp);
                return false;
...
static bool parse_str(char const *s, struct Spec_list *spec_list) {
  struct E_string es;
...
static bool homogeneous_spec_list(struct Spec_list *s) {
  int b, c;
  ...
  if (s2) {
    ...
    if (translating) {
      if (s2->has_equiv_class) {
        error(EXIT_FAILURE, 0, _("[=c=] expressions may not appear in string2\
 when translating"));
      }
...
\nstring2 must map all characters in the domain to one"));
    ...
    /* Not translating.  */
    {
      if (s2->n_indefinite_repeats > 0)
        error(EXIT_FAILURE, 0, _("the [c*] construct may appear in string2 only\
 when translating"));
    }
  }
}
...
static void set_initialize(struct Spec_list *s, bool complement_this_set,
                           bool *in_set) {
  int c;
  size_t i;
