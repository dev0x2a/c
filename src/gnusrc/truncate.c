...
static bool do_ftruncate(int fd, char const *fname, off_t ssize, off_t rsize,
                         rel_mode_t rel_mode) {
  struct stat sb;
  off_t nsize;
  ...
  if (block_mode) {
    ...
    if (ssize < OFF_T_MIN / blksize || ssize > OFF_T_MAX / blksize) {
      error(0, 0, _("overflow in %" PRIdMAX " * %" PRIdMAX
                    " byte blocks for file %s"),
            (intmax_t)ssize, (intmax_t)blksize, quoteaf(fname));
      return false;
    }
    ssize *= blksize;
  }
  if (rel_mode) {
    ...
    if (0 <= rsize)
      ...
      if (usable_st_size(&sb)) {
        ...
        if (file_size < 0) {
          error(0, errno, _("cannot get the size of %s"), quoteaf(fname));
          return false;
        }
      }
      fsize = file_size;
    ...
    /* 1..ssize -> ssize */
    {
      ...
      if (overflow > OFF_T_MAX) {
        error(0, 0, _("overflow rounding up size of file %s"), quoteaf(fname));
        return false;
      }
      nsize = overflow;
    } else {
...
int main(int argc, char **argv) {
  ...
  if (!ref_file && !got_size) {
    error(0, 0, _("you must specify either %s or %s"), quote_n(0, "--size"),
          quote_n(1, "--reference"));
    usage(EXIT_FAILURE);
  }
  ...
  if (block_mode && !got_size) {
    error(0, 0, _("%s was specified but %s was not"), quote_n(0, "--io-blocks"),
          quote_n(1, "--size"));
    usage(EXIT_FAILURE);
  }
  ...
  while ((fname = *argv++) != NULL) {
    if ((fd = open(fname, oflags, MODE_RW_UGO)) == -1) {
      ...
      if (!(no_create && errno == ENOENT)) {
        error(0, errno, _("cannot open %s for writing"), quoteaf(fname));
        errors = true;
      }
      continue;
    }
