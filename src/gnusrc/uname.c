...
static struct option const arch_long_options[] = {{GETOPT_HELP_OPTION_DECL},
                                                  {GETOPT_VERSION_OPTION_DECL},
                                                  {NULL, 0, NULL, 0}};
...
int main(int argc, char **argv) {
  ...
  if (toprint & (PRINT_KERNEL_NAME | PRINT_NODENAME | PRINT_KERNEL_RELEASE |
    ...
    if (element == unknown) {
      ...
      if (element == unknown) {
        ...
        if (sysctlbyname("hw.cputype", &cputype, &cs, NULL, 0) == 0 &&
            (ai = NXGetArchInfoFromCpuType(cputype, CPU_SUBTYPE_MULTIPLE)) !=
                NULL)
          element = ai->name;

        /* Hack "safely" around the ppc vs. powerpc return value. */
        if (cputype == CPU_TYPE_POWERPC && STRNCMP_LIT(element, "ppc") == 0)
          element = "powerpc";
      }
#endif
    }
  ...
  if (toprint & PRINT_HARDWARE_PLATFORM) {
...
#if HAVE_SYSINFO && defined SI_PLATFORM
    {
      ...
      if (0 <=
          sysinfo(SI_PLATFORM, hardware_platform, sizeof hardware_platform))
