...
static char const *const grouping_method_string[] = {"prepend", "append",
                                                     "separate", "both", NULL};
...
static void writeline(struct linebuffer const *line, bool match,
                      uintmax_t linecount) {
  if (!(linecount == 0
            ? output_unique
    ...
    if (optc == -1 || (posixly_correct && nfiles != 0) ||
      ...
      case 'D':
        ...
        if (optarg == NULL)
          ...
          delimit_groups = XARGMATCH("--all-repeated", optarg,
                                     delimit_method_string, delimit_method_map);
        output_option_used = true;
        break;
      ...
      case GROUP_OPTION:
        if (optarg == NULL)
          ...
          grouping = XARGMATCH("--group", optarg, grouping_method_string,
                               grouping_method_map);
        break;
      ...
      case 'w':
        check_chars =
            size_opt(optarg, N_("invalid number of bytes to compare"));
        break;
