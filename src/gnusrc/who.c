...
#else
#define PIDSTR_DECL_AND_INIT(Var, Utmp_ent) const char *Var = ""
#endif
...
static const char *time_string(const STRUCT_UTMP *utmp_ent) {
...
/* Print formatted output line. Uses mostly arbitrary field sizes, probably
   ...
   pids, etc. */
static void print_line(int userlen, const char *user, const char state,
                       int linelen, const char *line, const char *time_str,
                       const char *idle, const char *pid, const char *comment,
                       const char *exitstr) {
...
static bool is_tty_writable(struct stat const *pstat) {
#ifdef TTY_GROUP_NAME
...
static void print_deadprocs(const STRUCT_UTMP *utmp_ent) {
  ...
  if (!exitstr)
    exitstr = xmalloc(strlen(_("term=")) +
                      ...
                      strlen(_("exit=")) +
                      INT_STRLEN_BOUND(UT_EXIT_E_EXIT(utmp_ent)) + 1);
...
static void print_heading(void) {
  print_line(-1, _("NAME"), ' ', -1, _("LINE"), _("TIME"), _("IDLE"), _("PID"),
             _("COMMENT"), _("EXIT"));
}
