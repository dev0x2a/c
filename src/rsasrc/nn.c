...
static NN_DIGIT subdigitmult PROTO_LIST((NN_DIGIT *, NN_DIGIT *, NN_DIGIT,
 ...
 */
void NN_Decode(a, digits, b, len) NN_DIGIT *a;
unsigned char *b;
 ...
 */
void NN_Encode(a, len, b, digits) NN_DIGIT *b;
unsigned char *a;
unsigned int digits, len;
{
  NN_DIGIT t;
...
void NN_AssignZero(a, digits) NN_DIGIT *a;
unsigned int digits;
{
  if (digits) {
    do {
    ...
    if (s == MAX_NN_DIGIT)
      ...
      *(t + 1) -= v;
      ...
      if (cHigh == MAX_NN_HALF_DIGIT)
        ...
        aLow = (NN_HALF_DIGIT)((TO_HIGH_HALF(*(t + 1)) + HIGH_HALF(*t)) /
                               (cHigh + 1));
 ...
 */
void NN_ModMult(a, b, c, d, digits) NN_DIGIT *a, *b, *c, *d;
unsigned int digits;
{
  NN_DIGIT t[2 * MAX_NN_DIGITS];
 ...
 */
void NN_ModInv(a, b, c, digits) NN_DIGIT *a, *b, *c;
unsigned int digits;
{
...
void NN_Gcd(a, b, c, digits) NN_DIGIT *a, *b, *c;
unsigned int digits;
{
  short i;
...
int NN_Cmp(a, b, digits) NN_DIGIT *a, *b;
unsigned int digits;
{
...
int NN_Zero(a, digits) NN_DIGIT *a;
unsigned int digits;
{
  if (digits) {
    do {
...
void NN_Assign(a, b, digits) NN_DIGIT *a, *b;
unsigned int digits;
{
  if (digits) {
    do {
...
static NN_DIGIT subdigitmult(a, b, c, d, digits) NN_DIGIT *a, *b, c, *d;
unsigned int digits;
{
...
static void dmult(a, b, high, low) NN_DIGIT a, b;
NN_DIGIT *high;
NN_DIGIT *low;
{
