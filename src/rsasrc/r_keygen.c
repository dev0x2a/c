...
static int RSAFilter PROTO_LIST((NN_DIGIT *, unsigned int, NN_DIGIT *,
                                 unsigned int));
...
R_RANDOM_STRUCT *randomStruct;   /* random structure */
{
  ...
  NN_ASSIGN_DIGIT(e, protoKey->useFermat4 ? (NN_DIGIT)65537 : (NN_DIGIT)3,
                  nDigits);
  ...
  R_memcpy((POINTER)privateKey->modulus, (POINTER)publicKey->modulus,
           MAX_RSA_MODULUS_LEN);
  R_memcpy((POINTER)privateKey->publicExponent, (POINTER)publicKey->exponent,
           MAX_RSA_MODULUS_LEN);
...
static int RSAFilter(a, aDigits, b, bDigits) NN_DIGIT *a, *b;
