...
int R_RandomInit(random) R_RANDOM_STRUCT *random; /* new random structure */
{
...
int R_RandomUpdate(random, block,
                   len) R_RANDOM_STRUCT *random; /* random structure */
...
int R_GetRandomBytesNeeded(
    bytesNeeded,
    random) unsigned int *bytesNeeded; /* number of mix-in bytes needed */
...
int R_GenerateBytes(block, len, random) unsigned char *block; /* block */
unsigned int len;        /* length of block */
R_RANDOM_STRUCT *random; /* random structure */
{
  MD5_CTX context;
...
void R_RandomFinal(random) R_RANDOM_STRUCT *random; /* random structure */
...
void R_RandomCreate(random) R_RANDOM_STRUCT *random; /* random structure */
{
  clock_t cnow;
  time_t t;
  struct tm *gmt;
